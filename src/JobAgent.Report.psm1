...
function Add-JobAgentReportSearchInterfaceHtml {
    ...
    [void]$Lines.Add('const read=()=>{const p=new URLSearchParams(location.hash.slice(1)),rawPage=p.get("page"),requestedPage=Number(rawPage);return{view:p.get("view")==="companies"?"companies":"jobs",page:Math.max(1,requestedPage||1),_pageNeedsNormalization:rawPage!==null&&(!Number.isInteger(requestedPage)||requestedPage<1),q:p.get("q")||"",area:(p.get("area")||"").split(",").filter(Boolean),workModel:(p.get("workModel")||"").split(",").filter(Boolean),employmentType:(p.get("employmentType")||"").split(",").filter(Boolean),workTime:(p.get("workTime")||"").split(",").filter(Boolean),age:p.get("age")||""}};')
