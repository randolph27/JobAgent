...
function Add-JobAgentReportSearchInterfaceHtml {
    ...
    [void]$Lines.Add('<div class="search-tabs" role="tablist" aria-label="Bestandsansicht"><button type="button" id="jobagent-tab-jobs" role="tab" aria-selected="true" aria-controls="jobagent-jobs" data-jobagent-view="jobs">Stellen</button><button type="button" id="jobagent-tab-companies" role="tab" aria-selected="false" aria-controls="jobagent-companies" data-jobagent-view="companies">Firmen</button><button type="button" id="jobagent-tab-applications" role="tab" aria-selected="false" aria-controls="jobagent-applications" data-jobagent-view="applications">Bewerbungen</button></div>')
    ...
    [void]$Lines.Add('</div></div><div id="jobagent-companies" role="tabpanel" aria-labelledby="jobagent-tab-companies" hidden><div id="jobagent-company-results" class="result-list"></div></div><div id="jobagent-applications" role="tabpanel" aria-labelledby="jobagent-tab-applications" hidden><div id="jobagent-application-results" class="result-list"></div></div><nav id="jobagent-pagination" class="pagination" aria-label="Seitennavigation"></nav><p class="no-js-notice">Persoenliche Funktionen erfordern JavaScript und sind in dieser statischen Ansicht nicht verfuegbar.</p></div></div>')
    ...
    [void]$Lines.Add('}());')
    [void]$Lines.Add('</script>')
    [void]$Lines.Add('<script>(function(){const tab=document.getElementById("jobagent-tab-applications"),panel=document.getElementById("jobagent-applications"),jobsPanel=document.getElementById("jobagent-jobs"),companiesPanel=document.getElementById("jobagent-companies"),count=document.getElementById("jobagent-result-count"),pagination=document.getElementById("jobagent-pagination"),dataNode=document.getElementById("jobagent-search-data");if(!tab||!panel||!dataNode)return;const labels={PREPARING:"Vorbereiten",APPLIED:"Beworben",INTERVIEW:"Gespraech",REJECTED:"Absage",WITHDRAWN:"Zurueckgezogen"},render=()=>{let data,records={};try{data=JSON.parse(dataNode.textContent);const loaded=window.JobAgentUserState.read(window.localStorage);records=loaded.persistent?loaded.state.jobs:{}}catch{return}const known=new Map((data.jobs||[]).map(job=>[job.job_id,job])),entries=Object.entries(records).filter(([,record])=>record&&labels[record.application_stage]).map(([id,record])=>({job:known.get(id)||{job_id:id,title:record.reference&&record.reference.title||"Lokale Stellenreferenz",company:record.reference&&record.reference.company||"Keine Angabe"},record})).sort((a,b)=>a.job.job_id.localeCompare(b.job.job_id,"de")),root=panel.querySelector(".result-list");root.replaceChildren(...entries.map(entry=>{const card=document.createElement("article"),heading=document.createElement("h3"),stage=document.createElement("p"),next=document.createElement("p"),tasks=document.createElement("ul");card.className="result-card application-card";heading.textContent=entry.job.title+" · "+entry.job.company;stage.textContent="Status: "+labels[entry.record.application_stage];next.textContent=entry.record.next_action?"Naechste Aktion: "+entry.record.next_action:"Keine naechste Aktion gespeichert.";entry.record.tasks.filter(task=>task.deleted_at===null&&task.status==="OPEN").sort((a,b)=>a.local_date.localeCompare(b.local_date)||a.task_id.localeCompare(b.task_id)).forEach(task=>{const item=document.createElement("li");item.textContent="Offen: "+task.local_date+(task.time_with_offset?" "+task.time_with_offset:"")+" · "+task.title;tasks.append(item)});if(!tasks.childElementCount){const item=document.createElement("li");item.textContent="Keine offenen Termine.";tasks.append(item)}card.append(heading,stage,next,tasks);return card}));if(!entries.length){const empty=document.createElement("p");empty.className="empty-state";empty.textContent="Keine Bewerbungen mit aktivem Status.";root.append(empty)}count.textContent="Bewerbungen: "+entries.length+" Treffer.";pagination.replaceChildren()};tab.addEventListener("click",event=>{event.stopImmediatePropagation();event.preventDefault();jobsPanel.hidden=true;companiesPanel.hidden=true;panel.hidden=false;document.getElementById("jobagent-tab-jobs").setAttribute("aria-selected","false");document.getElementById("jobagent-tab-companies").setAttribute("aria-selected","false");tab.setAttribute("aria-selected","true");render()},{capture:true});window.addEventListener("storage",()=>{if(!panel.hidden)render()})}());</script></section>')
}
