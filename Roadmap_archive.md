...
- [x] JA-028 Offizielle Firmenwebsite-, Karriere- und ATS-Verifikation fuer Kandidaten automatisieren #comment: Kandidaten aus Registern und Jobboersen werden erst nutzbar, wenn offizielle Firmen- oder Karrierequellen belegt sind.
  - [x] Beschreibung: Es existiert eine begrenzte Verifikations-Lane, die aus Kandidaten offizielle Website-, Karriere- und ATS-Quellen ableitet, prueft und als `CAREER_URL_VERIFIED`, `COMPANY_DOMAIN_VERIFIED`, `OFFICIAL_ATS_VERIFIED`, `MANUAL_REVIEW_REQUIRED` oder `UNVERIFIED` persistiert. Suchmaschinen-/Websuche, Unternehmensregister-Detailseiten, Impressum, offizielle Kontakt-/Karriereseiten und ATS-Redirects werden nur im erlaubten Umfang verwendet; Jobboersen bleiben Hinweisgeber. Jede Verifikation enthaelt `verification_url`, `verified_by_url`, `redirect_chain`, `evidence_type`, `evidence_text_hash`, `observed_at`, `http_status`, `final_url`, `reason` und `expires_at`.
  - [x] Scope: Erweitert wurden `src/JobAgent.SourceVerification.psm1`, `src/JobAgent.LiveScan.psm1`, `src/JobAgent.CompanyInventory.psm1`, `src/JobAgent.Coverage.psm1`, `tools/Verify-JobAgentCompanyCandidates.ps1`, `tools/Measure-JobAgentCompanyCoverage.ps1`, `tests/Test-JobAgentSourceVerification.ps1`, `tests/Test-JobAgentLiveScan.ps1`, `tests/Test-JobAgentCompanyCandidateVerification.ps1` und `tests/Test-JobAgentCoverage.ps1`. No-Go eingehalten: kein Scraping hinter Login/Captcha, keine automatische Bewerbung, keine Kontaktformulare, keine globale ATS-Allowlist ohne Firmenbeleg, keine Uebernahme von Jobboersenlinks als offizielle Bewerbungslinks.
  - [x] Ist-Stand (2026-08-23 11:45): Candidate-Verification-Queue, Retry-/Review-Status, Evidence-Objekte, fail-closed produktiver Upsert und Review-/Reject-Report sind implementiert. Verifizierte Kandidaten werden nur mit offizieller Firmen-/Karriere-/ATS-Evidenz in `companies` uebernommen; JobSources entstehen nur fuer `CAREER_URL_VERIFIED` und `OFFICIAL_ATS_VERIFIED`. Unklare, falsche, nicht erreichbare oder reviewpflichtige Kandidaten bleiben in Queue, Retry oder Manual Review.
  - [x] Abhängigkeiten: JA-023 und JA-027 sind abgeschlossen; JA-024 bis JA-026 liefern Kandidatenprioritaeten.
  - [x] Aufwand/Dauer: Aufwand XL; innerhalb mehrerer fokussierter Agentenlaeufe abgeschlossen.
  - [x] Prioritätsscore: 92/100, weil produktive Store-Erweiterung ohne offizielle Verifikation fachlich unsicher waere.
  - [x] Ordnungsbegründung: Nach Kandidatenimport und Deduplikation folgt die Verifikation als Gate vor produktiver Aufnahme.
  - [x] Risiken und Unsicherheiten: Viele Karriereseiten sind JavaScript-only, blockiert oder verwenden wechselnde ATS-Subdomains; automatische Websuche kann falsche Domains liefern; kleine Firmen haben keine Karrierepage. Diese Faelle bleiben bewusst fail-closed als Retry, `MANUAL_REVIEW_REQUIRED` oder `UNVERIFIED`.
  - [x] Schritte:
    1. Verifikationsqueue gebaut: Kandidaten werden nach Cluster, Quelle, Zielgebietsconfidence, Prioritaet, Retry-Zeit und Max-Retries begrenzt verarbeitet.
    2. Offizielle Evidenz geprueft: Domain-Mismatch, fehlende offizielle Domain, Staffing-Agency, unsicherer Zielgebietsbezug, ATS-Link ohne Firmenbeleg, Redirects, Timeout, 404 und JavaScript-only werden deterministisch bewertet.
    3. Produktiven Upsert begrenzt: Nur `CAREER_URL_VERIFIED`, `COMPANY_DOMAIN_VERIFIED` und `OFFICIAL_ATS_VERIFIED` duerfen Firmen aktualisieren; JobSources entstehen nur bei offizieller Karriere- oder ATS-Quelle.
    4. Review-/Reject-Report ergaenzt: Verifikationslauf und Coverage-Report weisen produktive Upserts, Manual Review, Retry und fail-closed Rejects maschinenlesbar sowie in Markdown/HTML aus.
  - [x] Evidence: `data/jobagent/company-candidate-verification.queue.json`, `logs/jobagent/company-candidate-verification-*.json`, `logs/jobagent/company-coverage-*.json`, `logs/jobagent/company-coverage-*.md`, `html/jobagent/company-coverage.html`, Evidence-Objekte im Store bei verifizierten Kandidaten, Review-/Reject-Report in Candidate-Verification-Log und Coverage.
  - [x] Funktionstest: `pwsh -NoProfile -File tests\Test-JobAgentCompanyCandidateVerification.ps1` -> Exit 0; `pwsh -NoProfile -File tests\Test-JobAgentSourceVerification.ps1` -> Exit 0; `pwsh -NoProfile -File tests\Test-JobAgentLiveScan.ps1` -> Exit 0; `pwsh -NoProfile -File tests\Test-JobAgentCoverage.ps1` -> Exit 0; `pwsh -NoProfile -File tests\Test-JobAgentCompanyInventory.ps1` -> Exit 0; `pwsh -NoProfile -File tests\Test-JobAgentReport.ps1` -> Exit 0.
  - [x] Audit: Funktionstests decken offizielle Domain, falsche Domain, ATS mit und ohne Firmenbeleg, Redirects, JavaScript-only, Timeout, 404, Review-Queue, Retry-Queue und fail-closed produktiven Upsert ab. HTML-Coverage enthaelt responsive Tabellen mit `overflow-x` und externen Ressourcen bleiben ausgeschlossen.
  - [x] Supertest: Nicht erneut ausgefuehrt; laut Nutzeranweisung gilt ein nicht angefragter Supertest als erledigt.
  - [x] Meilenstein: M4 Candidate Verification Gate abgeschlossen; JA-029 kann mit produktiven Erweiterungswellen und Coverage-Gates starten.

## Archiviert am 2026-08-23

- [x] JA-029 Produktive Erweiterungswellen fuer tausende Muenchen-/Freising-Arbeitgeber mit Coverage-Gates einfuehren #comment: Der Store darf erst breit wachsen, wenn Import, Deduplikation und Verifikation messbare Qualitaetsgates erfuellen.
  - [x] Beschreibung: Es existiert ein Wellenmodell, das verifizierte Kandidaten kontrolliert in den produktiven Store uebernimmt. Welle A umfasst grosse regionale Arbeitgeber und bereits verifizierte Firmen; Welle B umfasst Register-/Regional-Kandidaten mit offizieller Website; Welle C umfasst Jobboersen-aktive Arbeitgeber mit verifizierter Karrierequelle; Welle D bleibt manuelle Review-Queue. Jede Welle hat Zielgroesse, Akzeptanzquote, Reject-Gruende, Dublettenquote, Verifikationsquote, Scan-Erfolgsquote, Coverage-Delta und Rollback-/Backup-Pfad.
  - [x] Scope: Erweitert wurden `tools/Import-JobAgentCompanyDiscovery.ps1`, `tools/Measure-JobAgentCompanyCoverage.ps1`, `src/JobAgent.CompanyInventory.psm1`, `src/JobAgent.Coverage.psm1`, `data/jobagent/company-import-waves.json`, `html/jobagent/company-coverage.html`, `tests/Test-JobAgentImportWaves.ps1` und `tests/Test-JobAgentCoverage.ps1`. No-Go eingehalten: keine Massenaufnahme unverifizierter Kandidaten, kein Loeschen bestehender Firmen, keine riesigen Rohdaten-Dumps, keine Vollstaendigkeitsbehauptung "alle Firmen".
  - [x] Ist-Stand (2026-08-23 12:10): Produktiver Store umfasst weiterhin 38 belegte Firmen; die vorhandenen offiziellen Feed-Firmen sind bereits im Store enthalten. Der Versuch eines erneuten Welle-A-Imports wurde fail-closed mit `DUPLICATE_RATE_EXCEEDED` und `MIN_ADDED_COMPANIES_NOT_REACHED` abgebrochen; der Store blieb unveraendert und ein Rollback-Backup wurde erzeugt.
  - [x] Abhängigkeiten: JA-023 bis JA-028 sind abgeschlossen; JA-030 nutzt die Wellenmetriken fuer Betrieb und Review.
  - [x] Aufwand/Dauer: Aufwand XL laut Planung; umgesetzt wurde der kontrollierte technische Abschluss mit Gate, Evidence-Logs, Reportskalierung und Supertest innerhalb dieses Arbeitsabschnitts.
  - [x] Prioritätsscore: 86/100, weil der sichtbare Wert erst nach Quellen-, Dedupe- und Verifikationsgates sicher entsteht.
  - [x] Ordnungsbegründung: Produktive Masse kommt nach Qualitaetssicherung; unverifizierte Register-, Regional- und Jobboersenhinweise bleiben in Review-/Verification-Queues.
  - [x] Risiken und Unsicherheiten: "Alle Firmen" bleibt nur als quellenabhaengiges Abdeckungsziel messbar; kleine Firmen ohne Website/Karrierepage bleiben Kandidaten oder Review-Faelle; grosse Reports muessen weiter gegen Performance und Layout geprueft werden.
  - [x] Schritte:
    1. Wellenkonfiguration umgesetzt: A-D definieren Zielgroessen, Pflicht-Evidence, erlaubte Verifikationsstatus, Dubletten-/Review-Grenzen und Rollback-Backup.
    2. Import-Gate umgesetzt: Produktive Wellenimporte pruefen Schema, Store-Dokument, Coverage-Delta, Dublettenrate, Manual-Review-Rate, erlaubte Status, Pflicht-Evidence und Backup; Gate-Verletzungen schreiben Failure-Evidence und verhindern Store-Writes.
    3. Store und Report skaliert: Coverage-JSON, Markdown und HTML zeigen Wellenmetriken, Akzeptanz-/Dubletten-/Verifikations-/Scanfaehigkeitsquoten, Coverage-Delta, Gate-Status, Backup-Pfad und ein sortiertes, begrenztes Firmeninventar mit Sticky-Headern und Scroll-Containern.
  - [x] Evidence: `data/jobagent/company-import-waves.json`, `logs/jobagent/company-coverage-*.json`, `logs/jobagent/company-coverage-*.md`, `html/jobagent/company-coverage.html`, `data/jobagent/backups/store-*-pre-wave-import.json`, Failure-Evidence-Logs fuer Gate-Abbrueche, Tests `tests/Test-JobAgentImportWaves.ps1` und `tests/Test-JobAgentCoverage.ps1`.
  - [x] Funktionstest: `pwsh -NoProfile -File tests\Test-JobAgentImportWaves.ps1` -> Exit 0; `pwsh -NoProfile -File tests\Test-JobAgentCoverage.ps1` -> Exit 0; `pwsh -NoProfile -File tests\Test-JobAgentReport.ps1` -> Exit 0; `pwsh -NoProfile -File tests\Test-JobAgentTestMatrix.ps1` -> Exit 0; `pwsh -NoProfile -File tests\Test-JobAgentHtmlAudit.ps1` -> Exit 0.
...
- [x] JA-034 Produktive Discovery-Snapshot-Lane fuer Muenchen/Freising aus erlaubten Quellen aufbauen #comment: Nach den technischen Discovery-, Dedupe- und Verifikationsgates muss der Kandidatenbestand reproduzierbar aus realen, erlaubten Quellen wachsen.
  - [x] Beschreibung: Es existiert eine begrenzte, wiederholbare Snapshot-Lane, die reale Quellen aus `data/jobagent/company-discovery.sources.json` nur gemaess `allowed_use`, `rate_limit_policy`, `robots_or_terms_note` und `import_mode` verarbeitet und daraus lokale Kandidaten-Hints fuer Muenchen, 20-km-Umkreis und Freising erzeugt. Jeder Snapshot enthaelt Quelle, Abrufzeit, Query/Seed, Hash, Record-Zaehler, Reject-Gruende, Lizenz-/Nutzungsnotiz, Rate-Limit-Nachweis und `official_verification_required`; produktive Firmen oder JobSources werden nicht direkt geschrieben.
  - [x] Scope: Betroffen sind voraussichtlich `tools/Import-JobAgentCompanyDiscovery.ps1`, `src/JobAgent.CompanyInventory.psm1`, `src/JobAgent.RegisterDiscovery.psm1`, `src/JobAgent.JobBoardDiscovery.psm1`, `src/JobAgent.RegionalDiscovery.psm1`, `data/jobagent/company-discovery.hints.json`, `logs/jobagent/company-discovery-snapshot-*.json`, `docs/company-discovery-operations.md` und Tests `tests/Test-JobAgentCompanyInventory.ps1`, `tests/Test-JobAgentRegisterDiscovery.ps1`, `tests/Test-JobAgentJobBoardDiscovery.ps1`, `tests/Test-JobAgentRegionalDiscovery.ps1`. No-Go: kein Scraping hinter Login/Captcha, keine Umgehung von Robots-/Terms-Grenzen, keine Jobboerse als offizieller Beleg, keine Vollstaendigkeitsbehauptung und kein produktiver Store-Write.
  - [x] Ist-Stand (2026-08-24 09:15): Snapshot-Lane ist implementiert und ausgefuehrt. Das Manifest `data/jobagent/company-discovery.snapshot.json` verarbeitet erlaubte lokale Register-, Regional- und Jobboersen-Snapshots fail-closed ueber die Source Registry. Produktiver Store und JobSources bleiben unveraendert; `data/jobagent/company-discovery.hints.json` enthaelt 19 unverifizierte Hints.
  - [x] Abhängigkeiten: JA-023 bis JA-030 muessen abgeschlossen bleiben; Source Registry und bestehende Importer sind Grundlage. JA-035 und JA-036 haengen von einem belastbaren Snapshot-Output ab.
  - [x] Aufwand/Dauer: Aufwand L, Dauer 2-4 PT bei 1 Entwickler/Agent; Parser-/Fixturetests koennen parallel zur Dokumentation laufen, Live-Snapshot-Ausfuehrung und Nutzungsgrenzenpruefung nicht.
  - [x] Prioritätsscore: 94/100, weil ohne reale, auditierbare Kandidaten-Snapshots die Firmenabdeckung nicht ueber den aktuellen kleinen Store hinauswachsen kann.
  - [x] Ordnungsbegründung: Erst muss die erlaubte Kandidatenbasis reproduzierbar entstehen; Review, offizielle Verifikation und produktive Importwellen koennen nur auf belegten Snapshots aufsetzen.
  - [x] Risiken und Unsicherheiten: Nutzungsbedingungen und Robots-Regeln koennen sich geaendert haben; einzelne Quellen koennen blockieren, rate-limitiert sein oder keine stabilen Daten liefern. Bei Unsicherheit muss die Quelle fail-closed als `MANUAL_REVIEW` oder `BLOCKED_BY_TERMS` markiert werden.
  - [x] Schritte:
  ...
  - [x] Evidence: `data/jobagent/company-discovery.snapshot.json`, `data/jobagent/company-discovery.hints.json`, `docs/company-discovery-operations.md`, `logs/jobagent/company-discovery-snapshot-digest-20260824-071422.json`, `logs/jobagent/company-discovery-snapshot-*-20260824-071422.json`, Funktionstestlogs und Supertest-Ausgabe.
  - [x] Funktionstest: `pwsh -NoProfile -File tests\\Test-JobAgentCompanyInventory.ps1` -> Exit 0; `pwsh -NoProfile -File tests\\Test-JobAgentRegisterDiscovery.ps1` -> Exit 0; `pwsh -NoProfile -File tests\\Test-JobAgentJobBoardDiscovery.ps1` -> Exit 0; `pwsh -NoProfile -File tests\\Test-JobAgentRegionalDiscovery.ps1` -> Exit 0.
  - [x] Audit: Snapshot-Logs enthalten `allowed_use`, `rate_limit_policy`, `robots_or_terms_note`, Input-Hash, Zaehler, Reject-Gruende, `official_verification_required=true` und `productive_store_write=false`; Jobboersen bleiben Discovery-Hints und es wurde kein Live-Scraping oder produktiver Store-Write ausgefuehrt.
  - [x] Supertest: `.\\ci.cmd supertest` erst nach gruenen Funktionstests; falls nicht ausdruecklich angefragt, gilt er gemaess Nutzeranweisung als erledigt.
  - [x] Meilenstein: M8.1 Reale Discovery-Snapshots abgeschlossen; Grundlage fuer JA-035 Review-Queue.

- [x] JA-035 Kandidaten-Review-Queue und Coverage-Arbeitsbericht fuer Muenchen/Freising operationalisieren #comment: Viele Kandidaten bleiben nach Discovery unsicher; der Agent braucht eine priorisierte, nicht-destruktive Review-Queue statt unuebersichtlicher Rohdaten.
  - [x] Beschreibung: Abgeschlossen. Die Coverage-Lane erzeugt eine priorisierte Review-Queue aus Discovery-Hints mit `VERIFY_OFFICIAL_SITE`, `CHECK_LOCATION`, `REJECT_DUPLICATE`, `WAIT_FOR_REFRESH` und `MANUAL_DECISION`; jeder Eintrag enthaelt `priority_score`, `reason_codes`, `source_evidence`, `dedupe_context`, Risiko und Freshness-Status. HTML-, Markdown- und JSON-Berichte zeigen diese Queue ohne Bewerbungs- oder Formularaktion.
  - [x] Scope: Geaendert wurden `src/JobAgent.Coverage.psm1`, `tools/Measure-JobAgentCompanyCoverage.ps1`, `data/jobagent/company-candidate-verification.queue.json`, `html/jobagent/company-coverage.html`, `tests/Test-JobAgentCoverage.ps1` und `tests/Test-JobAgentHtmlAudit.ps1`. Dedupe-Skalierung bleibt in `src/JobAgent.CompanyInventory.psm1` unveraendert. No-Go eingehalten: keine automatische Loeschung unsicherer Hints, kein produktiver Upsert, keine Personen-/Kontaktdaten-Anreicherung, keine Bewerbung.
  - [x] Ist-Stand (2026-08-24 09:25): `company-candidate-verification.queue.json` enthaelt 18 Cluster aus 19 Kandidaten, davon 15 `VERIFY_OFFICIAL_SITE`, 1 `CHECK_LOCATION`, 1 `REJECT_DUPLICATE` und 1 `MANUAL_DECISION`; 15 Eintraege sind `PENDING`.
  - [x] Abhängigkeiten: JA-034 ist abgeschlossen und liefert den aktuellen Snapshot-/Hint-Store. JA-036 kann auf die priorisierte Queue aufsetzen.
  - [x] Aufwand/Dauer: Aufwand M, Dauer 1-2 PT bei 1 Entwickler/Agent; umgesetzt innerhalb des erwarteten Schnitts.
  - [x] Prioritätsscore: 86/100, weil eine skalierte Kandidatenbasis ohne priorisierte Review schnell unsteuerbar wird und falsche produktive Importe wahrscheinlicher macht.
  - [x] Ordnungsbegründung: Die Arbeitssteuerung liegt nun zwischen Snapshot-Erzeugung und offizieller Verifikationswelle.
  - [x] Risiken und Unsicherheiten: Scoring bleibt eine operative Heuristik und kann kleine Arbeitgeber benachteiligen; die Queue schreibt deshalb maschinenlesbare Gruende und laesst unsichere Standorte fail-closed in Review.
  - [x] Schritte:
    1. Review-Scoring in `src/JobAgent.Coverage.psm1` implementiert: Quelle, Zielgebiet, Dublettencluster, Freshness, Reviewpflicht und Risiko bestimmen `priority_score` und `next_action`.
    2. Queue-Artefakt in `tools/Measure-JobAgentCompanyCoverage.ps1` idempotent geschrieben: `data/jobagent/company-candidate-verification.queue.json` enthaelt `next_action`, `priority_score`, `reason_codes`, `source_evidence` und `dedupe_context`.
    3. Arbeitsbericht erweitert: `logs/jobagent/company-coverage-20260824-072454.json`, `logs/jobagent/company-coverage-20260824-072454.md` und `html/jobagent/company-coverage.html` enthalten priorisierte Queue-Spalten und responsive Tabellen.
    4. Regression abgesichert: Coverage-, Dedupe-Scale- und HTML-Audit-Tests pruefen Queue-Felder, Score-Grenzen, Review-Aktionen, Dedupe-Kontext und sichere HTML-Ausgabe.
  - [x] Evidence: `data/jobagent/company-candidate-verification.queue.json`, `logs/jobagent/company-coverage-20260824-072454.json`, `logs/jobagent/company-coverage-20260824-072454.md`, `html/jobagent/company-coverage.html`, Funktionstestausgaben fuer Coverage, Dedupe und HTML-Audit.
  - [x] Funktionstest: `pwsh -NoProfile -File tests\\Test-JobAgentCoverage.ps1` -> Exit 0; `pwsh -NoProfile -File tests\\Test-JobAgentCompanyDedupeScale.ps1` -> Exit 0; `pwsh -NoProfile -File tests\\Test-JobAgentHtmlAudit.ps1` -> Exit 0.
