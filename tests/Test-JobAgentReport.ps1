...
$document.jobs[4].job_validity = [pscustomobject]@{ result = 'VALID'; reasons = @('Offizielle Detail-URL und Titel sind belegt.'); evaluated_at = '2026-08-17T10:00:00.000Z' }
...
Assert-True -Condition ($report.statistics.captured_jobs_total -eq 5) -Message 'Report trennt erfasste Stellen nicht von abgelehnten Rohdaten.'
...
foreach ($rawLabel in @('checked_jobs', 'active_matching_jobs', 'uncertain_sources', 'unreachable_career_pages')) {
...
foreach ($expected in @('<!DOCTYPE html>', '<h1 id="jobagent-search-heading">Stellenangebote</h1>', '<h2>Datenstand und Quellen</h2>', '<h2>Erfassungsscope und Vollstaendigkeit</h2>', '<h2>Neue passende Stellen</h2>', '<h2>Fehler und unsichere Quellen</h2>', '<h3>Quellenbestand</h3>', 'Scope', 'ALL_ROLES', 'Erfasste Stellen gesamt', 'Profiltreffer gesamt', 'Firmen gesamt', 'Firmen im Lauf', 'Faellige Firmen', 'Uebersprungene Firmen', 'Limit', 'Auswahlgrund', 'Faellig nach next_scan_at, danach Prioritaet', 'Quellen gesamt', 'Offizielle Quellen', 'Im letzten Lauf gescannt', 'Zeitbelege und Herkunft', 'Letzte vollstaendige Listenpruefung', '<th>Titel</th><th>Firma</th><th>Standort</th><th>Prioritaet</th><th>Status</th><th>Offizielle Stellen-URL</th><th>Karriere-URL</th><th>Quelle</th>', '120000 EUR', 'Budgetverantwortung', 'Kurzprofil', 'Beschreibung', 'href="https://beta_ag.example.invalid/careers" target="_blank" rel="noopener noreferrer">Quelle</a>', 'href="https://alpha_ag.example.invalid/jobs/alpha_new" target="_blank" rel="noopener noreferrer">Offizielle Stellen-URL</a>', 'href="https://alpha_ag.example.invalid/careers" target="_blank" rel="noopener noreferrer">Karriere-URL</a>', 'href="https://alpha_ag.example.invalid/careers" target="_blank" rel="noopener noreferrer">Karriere</a>', 'href="https://jobs.alpha_ag.example.invalid/search" target="_blank" rel="noopener noreferrer">ATS</a>')) {
    Assert-True -Condition ($html.Contains($expected)) -Message "HTML-Report enthaelt erwarteten Inhalt nicht: $expected"
}
foreach ($expected in @('id="jobagent-search"', 'id="jobagent-query"', 'id="jobagent-company"', 'id="jobagent-category"', 'id="jobagent-area"', 'id="jobagent-work-model"', 'id="jobagent-employment-type"', 'id="jobagent-work-time"', 'id="jobagent-age"', 'id="jobagent-favorites"', 'id="jobagent-applied"', 'id="jobagent-sort"', 'id="jobagent-pagination"', 'id="jobagent-tab-applications"', 'id="jobagent-applications"', 'id="jobagent-application-filters"', 'id="jobagent-application-query"', 'id="jobagent-application-stage"', 'id="jobagent-application-due"', 'id="jobagent-application-sort"', 'id="jobagent-visibility"', 'Ausblendungen verwalten', 'Nicht interessant', 'Stellen dieses Arbeitgebers ausblenden', 'Bewerbungen', 'Software Engineer')) {
...
foreach ($expected in @('class="jobboard-layout"', 'class="filter-region"', 'data-job-id="job:alpha_new"', 'Original-Stellenanzeige', 'Persoenliche Funktionen erfordern JavaScript und sind in dieser statischen Ansicht nicht verfuegbar.')) {
    Assert-True -Condition ($html.Contains($expected)) -Message "HTML-Report enthaelt den Stellenboersenvertrag nicht: $expected"
}
Assert-True -Condition ($html.IndexOf('<section id="jobagent-search"') -lt $html.IndexOf('<section id="jobagent-data-status"')) -Message 'Die Stellenboerse muss vor Datenstand und Quellen stehen.'
