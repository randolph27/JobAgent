...
'@
    ...
    Set-JobAgentLocationHash -WorkingDirectory $artifactRoot -SessionName $sessionName -Segments @('view=jobs', 'job=job:freising-pflege')
    ...
    Assert-True -Condition ($detailMarksBefore.hash -eq '#view=jobs&job=job%3Afreising-pflege') -Message 'Detailansicht: Der Job-Hash wurde nicht erhalten.'
    ...
    Assert-True -Condition ($detailAppliedResult.hash -eq '#view=jobs&job=job%3Afreising-pflege') -Message 'Detailansicht: Die Bewerbungsmarkierung hat unerwartet navigiert.'
    ...
    Assert-True -Condition ($detailReturnFocus.hash -eq '#view=jobs') -Message 'Detailansicht: Die Rueckkehr zur Trefferliste entfernt nicht nur den Job-Hash.'
    ...
    Assert-True -Condition ($cardFavoriteResult.hash -eq '#view=jobs') -Message 'Trefferkarte: Die Favoritenmarkierung hat unerwartet navigiert.'
