...
function Get-JobAgentGeometryMeasurement {
    ...
    $script = @"
() => { const s=(...codes)=>String.fromCharCode(...codes),tags=[s(98,117,116,116,111,110),s(105,110,112,117,116),s(115,101,108,101,99,116)],textTags=[s(104,49),s(104,50),s(104,51),s(112),s(97),s(98,117,116,116,111,110),s(108,97,98,101,108),s(111,112,116,105,111,110)],elements=Array.from(document.getElementsByTagName(s(42))); document.documentElement.style.zoom=$zoomValue; const visible=element=>{const style=getComputedStyle(element),rect=element.getBoundingClientRect();return !element.closest("details:not([open])")&&style.display!==s(110,111,110,101)&&style.visibility!==s(104,105,100,100,101,110)&&rect.width>0&&rect.height>0}; const controls=elements.filter(element=>tags.includes(element.tagName.toLowerCase())&&visible(element)).map((element,index)=>{const rect=element.getBoundingClientRect();return {id:element.id||element.name||String(element.textContent||String()).trim()||s(99,111,110,116,114,111,108,45)+index,left:rect.left,top:rect.top,right:rect.right,bottom:rect.bottom,width:rect.width,height:rect.height}}).filter(item=>item.width>0&&item.height>0); const invalidControls=controls.filter(item=>item.width<44||item.height<44||item.left<0||item.right>window.innerWidth+1); const overlaps=[]; for(let left=0;left<controls.length;left++){for(let right=left+1;right<controls.length;right++){const a=controls[left],b=controls[right];if(Math.min(a.right,b.right)-Math.max(a.left,b.left)>1&&Math.min(a.bottom,b.bottom)-Math.max(a.top,b.top)>1){overlaps.push({first:a.id,second:b.id})}}} const clippedText=elements.filter(element=>textTags.includes(element.tagName.toLowerCase())).filter(element=>{const style=getComputedStyle(element);return visible(element)&&style.overflow!==s(118,105,115,105,98,108,101)&&element.scrollWidth>element.clientWidth+1&&!element.title}).map(element=>String(element.textContent||String()).trim()).filter(Boolean); return JSON.stringify({viewport_width:window.innerWidth,viewport_height:window.innerHeight,root_scroll_width:document.documentElement.scrollWidth,invalid_controls:invalidControls,overlaps:overlaps,clipped_text:clippedText,controls:controls}); }
"@
...
function Assert-JobAgentGeometryMeasurement {
    ...
    $overlapNames = @($Measurement.overlaps | ForEach-Object { "$($_.first) <> $($_.second)" }) -join '; '
    Assert-True -Condition (@($Measurement.overlaps).Count -eq 0) -Message "${Case}: Sichtbare bedienbare Controls ueberlappen sich: $overlapNames"
...
function Invoke-JobAgentFilterReset {
    param(
        [Parameter(Mandatory)][string]$WorkingDirectory,
        [Parameter(Mandatory)][string]$SessionName,
        [Parameter(Mandatory)][string]$Case
    ...
    $focus = Get-JobAgentSessionValue -WorkingDirectory $WorkingDirectory -SessionName $SessionName -Case $Case -Script '() => { const reset=document.getElementById(String.fromCharCode(106,111,98,97,103,101,110,116,45,114,101,115,101,116)); if(!reset)throw new Error("Filter-Reset fehlt"); reset.focus(); return JSON.stringify({focused:document.activeElement===reset}); }'
    Assert-True -Condition ([bool]$focus.focused) -Message "${Case}: Der Filter-Reset kann nicht per Tastatur fokussiert werden."
    Invoke-JobAgentPlaywrightCli -WorkingDirectory $WorkingDirectory -Arguments @('--session', $SessionName, 'press', 'Space') | Out-Null
    Get-JobAgentSessionValue -WorkingDirectory $WorkingDirectory -SessionName $SessionName -Case $Case -Script 'async () => { await new Promise(resolve=>setTimeout(resolve,50)); return JSON.stringify({ready:true}); }' | Out-Null
}

function Set-JobAgentSelectValues {
    param(
        [Parameter(Mandatory)][string]$WorkingDirectory,
        [Parameter(Mandatory)][string]$SessionName,
    ...
    $valuesJson = '[' + (($Values | ForEach-Object { ConvertTo-Json -InputObject $_ -Compress }) -join ',') + ']'
    $script = "() => { const node=document.getElementById('$ElementId'); if(!node)throw new Error('Filterfeld fehlt: $ElementId'); const values=new Set($valuesJson); node.focus(); return JSON.stringify({focused:document.activeElement===node,available:Array.from(values).every(value=>Array.from(node.options).some(option=>option.value===value))}); }"
    $result = Get-JobAgentSessionValue -WorkingDirectory $WorkingDirectory -SessionName $SessionName -Case $Case -Script $script
    Assert-True -Condition ([bool]$result.focused) -Message "${Case}: Das Filterfeld '$ElementId' kann nicht fokussiert werden."
    Assert-True -Condition ([bool]$result.available) -Message "${Case}: Der erwartete Filterwert ist nicht verfuegbar."
}
...
function Set-JobAgentLocationHash {
    ...
    Get-JobAgentSessionValue -WorkingDirectory $WorkingDirectory -SessionName $SessionName -Case 'Hashnavigation' -Script 'async () => { await new Promise(resolve=>setTimeout(resolve,250)); return JSON.stringify({ready:true}); }' | Out-Null
}
...
try {
    ...
    if ($SavedSearchOnly) {
        ...
        $unexpectedExternalResources = @($savedSearch.external_resources | Where-Object { $_ -notmatch '^https?://gc\.kis\.v2\.scr\.kaspersky-labs\.com/' })
        Assert-True -Condition ($unexpectedExternalResources.Count -eq 0) -Message ('Gespeicherte Suche: Lokale Bedienung hat externe Netzwerkressourcen geladen: ' + ($unexpectedExternalResources -join ', '))
        $screenshots = @(
    ...
    if ($VisibilityOnly) {
        ...
        Assert-True -Condition ([bool]$visibility.management -and [string]$visibility.count -match '262 sichtbare Treffer, 2 durch Ausblendung ausgeschlossen') -Message 'Ausblendung: Verwaltung oder exakter Sichtbarkeitszaehler fehlt.'
    ...
    Assert-True -Condition (@($accessibility.tabs).Count -eq 4) -Message 'Die Tabansicht muss genau vier semantische Tabs enthalten.'
...
'@
    ...
    $applicationOverview = Get-JobAgentSessionValue -WorkingDirectory $artifactRoot -SessionName $sessionName -Case 'Bewerbungsuebersicht: lokale Stufen-, Termin- und Notizfilter' -Script '() => { const api=window.JobAgentUserState,job={job_id:"job:remote-contract",title:"Remote Vertrag Spezialistin",company:"Firma 006",official_url:"https://jobs.example.invalid/job/remote-contract"}; api.transitionApplication(localStorage,job,"INTERVIEW",null,"2026-09-15T12:04:00.000Z"); api.updateApplicationText(localStorage,job,{note:"Rueckruf mit Personalteam",next_action:"Unterlagen pruefen"},"2026-09-15T12:05:00.000Z"); api.upsertTask(localStorage,job,{task_id:"task_remote_follow_up",type:"FOLLOW_UP",title:"Rueckruf",local_date:"2026-09-16",time_with_offset:"09:00+02:00",status:"OPEN"},"2026-09-15T12:06:00.000Z"); const resourcesBefore=new Set(performance.getEntriesByType("resource").map(entry=>entry.name)); document.getElementById("jobagent-tab-applications").click(); const query=document.getElementById("jobagent-application-query"),stage=document.getElementById("jobagent-application-stage"),due=document.getElementById("jobagent-application-due"),sort=document.getElementById("jobagent-application-sort"),root=document.getElementById("jobagent-application-results"); query.value="Rueckruf"; query.dispatchEvent(new Event("input",{bubbles:true})); const queryIds=Array.from(root.querySelectorAll("[data-job-id]")).map(item=>item.dataset.jobId); stage.value="INTERVIEW"; stage.dispatchEvent(new Event("change",{bubbles:true})); due.value="next_7"; due.dispatchEvent(new Event("change",{bubbles:true})); sort.value="due_then_id"; sort.dispatchEvent(new Event("change",{bubbles:true})); const filteredIds=Array.from(root.querySelectorAll("[data-job-id]")).map(item=>item.dataset.jobId),text=root.textContent; document.getElementById("jobagent-application-filters").reset(); document.getElementById("jobagent-tab-jobs").click(); return JSON.stringify({query_ids:queryIds,filtered_ids:filteredIds,text,selected_tab:document.getElementById("jobagent-tab-applications").getAttribute("aria-selected"),network_entries:performance.getEntriesByType("resource").map(entry=>entry.name).filter(name=>!resourcesBefore.has(name))}); }'
    Assert-True -Condition ((@($applicationOverview.query_ids) -join ',') -eq 'job:remote-contract') -Message 'Bewerbungsuebersicht: Die lokale Notizsuche liefert nicht exakt die erwartete Stelle.'
    Assert-True -Condition ((@($applicationOverview.filtered_ids) -join ',') -eq 'job:remote-contract') -Message 'Bewerbungsuebersicht: Stufen- oder Faelligkeitsfilter liefert nicht exakt die erwartete Stelle.'
    Assert-True -Condition ($applicationOverview.text -match '2026-09-16' -and $applicationOverview.text -match '09:00\+02:00' -and $applicationOverview.text -match 'Rueckruf') -Message 'Bewerbungsuebersicht: Der offene Termin mit Offset fehlt.'
    Assert-True -Condition (@($applicationOverview.network_entries).Count -eq 0) -Message 'Bewerbungsuebersicht: Eine lokale Filteraktion hat eine externe Netzwerkressource geladen.'
    $caseEvidence.Add([pscustomobject]@{ case_id = 'ja052_application_overview_local_note_stage_due_and_sort_filters'; query_job_ids = @($applicationOverview.query_ids); filtered_job_ids = @($applicationOverview.filtered_ids); no_external_network = $true })
    ...
    Set-JobAgentLocationHash -WorkingDirectory $artifactRoot -SessionName $sessionName -Segments @('view=jobs', 'page=6')
    $detailOpenFocus = Get-JobAgentSessionValue -WorkingDirectory $artifactRoot -SessionName $sessionName -Case 'Detailansicht aus sichtbarer Trefferkarte vorbereiten' -Script '() => { const button=document.querySelector("[data-job-id=\"job:freising-pflege\"] .job-detail-open"); if(!button)throw new Error("Stellentitel auf der erwarteten Seite fehlt"); button.focus(); return JSON.stringify({focused:document.activeElement===button}); }'
    Assert-True -Condition ([bool]$detailOpenFocus.focused) -Message 'Detailansicht: Der Stellentitel kann nicht per Tastatur fokussiert werden.'
    Invoke-JobAgentPlaywrightCli -WorkingDirectory $artifactRoot -Arguments @('--session', $sessionName, 'press', 'Enter') | Out-Null
    $detailMarksBefore = Get-JobAgentSessionValue -WorkingDirectory $artifactRoot -SessionName $sessionName -Case 'Detailansicht mit getrennten Markierungen' -Script '() => { const card=document.querySelector(".job-detail"),marks=card?Array.from(card.querySelectorAll("[data-job-mark]")):[]; return JSON.stringify({hash:location.hash,job_id:card&&card.dataset.jobId,marks:marks.map(button=>({field:button.dataset.jobMark,pressed:button.getAttribute("aria-pressed"),label:button.getAttribute("aria-label")}))}); }'
    Assert-True -Condition ($detailMarksBefore.hash -eq '#view=jobs&page=6&job=job%3Afreising-pflege') -Message 'Detailansicht: Der Job-Hash oder der Seitenkontext wurde nicht erhalten.'
    Assert-True -Condition ($detailMarksBefore.job_id -eq 'job:freising-pflege') -Message 'Detailansicht: Die angeforderte Stelle wird nicht gerendert.'
    Assert-True -Condition (@($detailMarksBefore.marks | Where-Object { $_.field -eq 'favorite' -and $_.pressed -eq 'true' -and $_.label -eq 'Favorit entfernen' }).Count -eq 1) -Message 'Detailansicht: Die Favoritenmarkierung ist nicht eindeutig aktiv.'
    Assert-True -Condition (@($detailMarksBefore.marks | Where-Object { $_.field -eq 'applied' -and $_.pressed -eq 'false' -and $_.label -eq 'Als schon beworben markieren' }).Count -eq 1) -Message 'Detailansicht: Die Bewerbungsmarkierung ist nicht eindeutig inaktiv.'
    $detailAppliedResult = Get-JobAgentSessionValue -WorkingDirectory $artifactRoot -SessionName $sessionName -Case 'Detailansicht: Bewerbungsmarkierung aendert nur ihr eigenes Feld' -Script 'async () => { const detail=document.querySelector(".job-detail"),applied=detail&&detail.querySelector("[data-job-mark=applied]"); if(!applied)throw new Error("Bewerbungsmarkierung im Detail fehlt"); applied.click(); await new Promise(resolve=>setTimeout(resolve,50)); const record=window.JobAgentUserState.read(localStorage).state.jobs["job:freising-pflege"],marks=Array.from(document.querySelectorAll(".job-detail [data-job-mark]")).map(button=>({field:button.dataset.jobMark,pressed:button.getAttribute("aria-pressed")})); return JSON.stringify({hash:location.hash,record,marks,message:document.querySelector(".job-detail .job-mark-message").textContent}); }'
    Assert-True -Condition ($detailAppliedResult.hash -eq '#view=jobs&page=6&job=job%3Afreising-pflege') -Message 'Detailansicht: Die Bewerbungsmarkierung hat unerwartet navigiert.'
    Assert-True -Condition ([bool]$detailAppliedResult.record.favorite -and [bool]$detailAppliedResult.record.applied) -Message 'Detailansicht: Die Bewerbungsmarkierung hat Favorit und Bewerbung nicht getrennt gespeichert.'
    Assert-True -Condition (@($detailAppliedResult.marks | Where-Object { $_.field -eq 'favorite' -and $_.pressed -eq 'true' }).Count -eq 1) -Message 'Detailansicht: Der Favoritenbutton verlor seinen Zustand nach der Bewerbungsmarkierung.'
    Assert-True -Condition (@($detailAppliedResult.marks | Where-Object { $_.field -eq 'applied' -and $_.pressed -eq 'true' }).Count -eq 1) -Message 'Detailansicht: Der Bewerbungsbutton wurde nach dem Speichern nicht synchronisiert.'
    Assert-True -Condition ($detailAppliedResult.message -eq 'Bewerbungsmarkierung lokal gespeichert.') -Message 'Detailansicht: Das Speicherergebnis der Bewerbungsmarkierung fehlt oder ist falsch.'
    $detailScreenshotPath = Join-Path $artifactRoot 'JA-048-detail-1366.png'
    Invoke-JobAgentPlaywrightCli -WorkingDirectory $artifactRoot -Arguments @('--session', $sessionName, 'resize', 1366, 900) | Out-Null
    Invoke-JobAgentPlaywrightCli -WorkingDirectory $artifactRoot -Arguments @('--session', $sessionName, 'screenshot', '--filename', $detailScreenshotPath) | Out-Null
    Assert-True -Condition (Test-Path -LiteralPath $detailScreenshotPath) -Message 'Detailansicht: Der 1366-Pixel-Nachweisscreenshot fehlt.'
    Assert-True -Condition ((Get-Item -LiteralPath $detailScreenshotPath).Length -gt 10000) -Message 'Detailansicht: Der 1366-Pixel-Nachweisscreenshot ist unplausibel klein.'
    $screenshots.Add($detailScreenshotPath)
    Get-JobAgentSessionValue -WorkingDirectory $artifactRoot -SessionName $sessionName -Case 'Rueckkehr aus Detailansicht' -Script '() => { const back=document.querySelector(".job-detail button"); if(!back)throw new Error("Ruecktaste im Detail fehlt"); back.click(); return JSON.stringify({hash:location.hash}); }' | Out-Null
    $detailReturnFocus = Get-JobAgentSessionValue -WorkingDirectory $artifactRoot -SessionName $sessionName -Case 'Rueckfokus aus Detailansicht' -Script 'async () => { await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve))); const card=document.querySelector("[data-job-id=\"job:freising-pflege\"]"),marks=card?Array.from(card.querySelectorAll("[data-job-mark]")).map(button=>({field:button.dataset.jobMark,pressed:button.getAttribute("aria-pressed")})):[]; return JSON.stringify({hash:location.hash,focus:document.activeElement&&document.activeElement.className,tag:document.activeElement&&document.activeElement.tagName,id:document.activeElement&&document.activeElement.id,marks}); }'
    Assert-True -Condition ($detailReturnFocus.hash -eq '#view=jobs&page=6') -Message 'Detailansicht: Die Rueckkehr zur Trefferliste entfernt nicht nur den Job-Hash.'
    Assert-True -Condition ($detailReturnFocus.focus -eq 'job-detail-open') -Message "Detailansicht: Der Fokus kehrt nicht zum zuvor geoeffneten Stellentitel zurueck (erhalten: '$($detailReturnFocus.focus)', tag: '$($detailReturnFocus.tag)', id: '$($detailReturnFocus.id)')."
    Assert-True -Condition (@($detailReturnFocus.marks | Where-Object { $_.field -eq 'favorite' -and $_.pressed -eq 'true' }).Count -eq 1) -Message 'Trefferkarte: Der Favoritenzustand ist nach der Detailaktion nicht synchronisiert.'
    Assert-True -Condition (@($detailReturnFocus.marks | Where-Object { $_.field -eq 'applied' -and $_.pressed -eq 'true' }).Count -eq 1) -Message 'Trefferkarte: Der Bewerbungszustand ist nach der Detailaktion nicht synchronisiert.'
    $cardFavoriteResult = Get-JobAgentSessionValue -WorkingDirectory $artifactRoot -SessionName $sessionName -Case 'Trefferkarte: Favoritenmarkierung aendert nur ihr eigenes Feld' -Script 'async () => { const card=document.querySelector("[data-job-id=\"job:freising-pflege\"]"),favorite=card&&card.querySelector("[data-job-mark=favorite]"); if(!favorite)throw new Error("Favoritenmarkierung auf Trefferkarte fehlt"); favorite.click(); await new Promise(resolve=>setTimeout(resolve,50)); const record=window.JobAgentUserState.read(localStorage).state.jobs["job:freising-pflege"],marks=Array.from(document.querySelectorAll("[data-job-id=\"job:freising-pflege\"] [data-job-mark]")).map(button=>({field:button.dataset.jobMark,pressed:button.getAttribute("aria-pressed")})); return JSON.stringify({hash:location.hash,record,marks,message:card.querySelector(".job-mark-message").textContent}); }'
    Assert-True -Condition ($cardFavoriteResult.hash -eq '#view=jobs&page=6') -Message 'Trefferkarte: Die Favoritenmarkierung hat unerwartet navigiert.'
    Assert-True -Condition (-not [bool]$cardFavoriteResult.record.favorite -and [bool]$cardFavoriteResult.record.applied) -Message 'Trefferkarte: Die Favoritenmarkierung hat die Bewerbungsmarkierung nicht getrennt behandelt.'
    Assert-True -Condition (@($cardFavoriteResult.marks | Where-Object { $_.field -eq 'favorite' -and $_.pressed -eq 'false' }).Count -eq 1) -Message 'Trefferkarte: Der Favoritenbutton wurde nach dem Speichern nicht synchronisiert.'
    Assert-True -Condition (@($cardFavoriteResult.marks | Where-Object { $_.field -eq 'applied' -and $_.pressed -eq 'true' }).Count -eq 1) -Message 'Trefferkarte: Der Bewerbungsbutton verlor seinen Zustand nach der Favoritenaktion.'
    Assert-True -Condition ($cardFavoriteResult.message -eq 'Favorit lokal gespeichert.') -Message 'Trefferkarte: Das Speicherergebnis der Favoritenmarkierung fehlt oder ist falsch.'
    $starsScreenshotPath = Join-Path $artifactRoot 'JA-048-stars-390.png'
    Invoke-JobAgentPlaywrightCli -WorkingDirectory $artifactRoot -Arguments @('--session', $sessionName, 'resize', 390, 844) | Out-Null
    Invoke-JobAgentPlaywrightCli -WorkingDirectory $artifactRoot -Arguments @('--session', $sessionName, 'screenshot', '--filename', $starsScreenshotPath) | Out-Null
    Assert-True -Condition (Test-Path -LiteralPath $starsScreenshotPath) -Message 'Trefferkarte: Der 390-Pixel-Nachweisscreenshot fehlt.'
    Assert-True -Condition ((Get-Item -LiteralPath $starsScreenshotPath).Length -gt 10000) -Message 'Trefferkarte: Der 390-Pixel-Nachweisscreenshot ist unplausibel klein.'
    $screenshots.Add($starsScreenshotPath)
    $caseEvidence.Add([pscustomobject]@{ case_id = 'ja048_detail_and_card_marks_are_independent_and_keep_focus'; job_id = 'job:freising-pflege'; detail_before = $detailMarksBefore; detail_applied = $detailAppliedResult; return_focus = $detailReturnFocus.focus; card_favorite = $cardFavoriteResult })
    ...
    Set-JobAgentLocationHash -WorkingDirectory $artifactRoot -SessionName $sessionName -Segments @('view=jobs', 'area=FREISING_CITY', 'workModel=HYBRID', 'employmentType=PART_TIME')
    $snapshot = Get-JobAgentCliSnapshot -WorkingDirectory $artifactRoot -SessionName $sessionName
    ...
    Invoke-JobAgentFilterReset -WorkingDirectory $artifactRoot -SessionName $sessionName -Case 'Reset vor normalisierter Freitextsuche'
    $snapshot = Get-JobAgentCliSnapshot -WorkingDirectory $artifactRoot -SessionName $sessionName
    $queryRef = Get-JobAgentCliRef -Snapshot $snapshot -Roles @('searchbox', 'textbox') -Name 'Was?'
    ...
    foreach ($facetCase in @(
        ...
        $elementId = @{
            'Gebiet' = 'jobagent-area'
            'Arbeitsmodell' = 'jobagent-work-model'
            'Anstellungsart' = 'jobagent-employment-type'
            'Arbeitszeit' = 'jobagent-work-time'
            'Aktualitaet' = 'jobagent-age'
        }[$facetCase.control]
        $hashKey = @{
            'Gebiet' = 'area'
            'Arbeitsmodell' = 'workModel'
            'Anstellungsart' = 'employmentType'
            'Arbeitszeit' = 'workTime'
            'Aktualitaet' = 'age'
        }[$facetCase.control]
        Set-JobAgentSelectValues -WorkingDirectory $artifactRoot -SessionName $sessionName -ElementId $elementId -Values @($facetCase.value) -Case "Facet $($facetCase.control)=$($facetCase.value)"
        $facetHash = '#view=jobs&' + $hashKey + '=' + $facetCase.value
        Set-JobAgentLocationHash -WorkingDirectory $artifactRoot -SessionName $sessionName -Segments @('view=jobs', ('{0}={1}' -f $hashKey, $facetCase.value))
    ...
    Invoke-JobAgentFilterReset -WorkingDirectory $artifactRoot -SessionName $sessionName -Case 'Reset vor Unicode-Freitextsuche'
    $snapshot = Get-JobAgentCliSnapshot -WorkingDirectory $artifactRoot -SessionName $sessionName
    $queryRef = Get-JobAgentCliRef -Snapshot $snapshot -Roles @('searchbox', 'textbox') -Name 'Was?'
    ...
    $snapshot = Get-JobAgentCliSnapshot -WorkingDirectory $artifactRoot -SessionName $sessionName
    Invoke-JobAgentFilterReset -WorkingDirectory $artifactRoot -SessionName $sessionName -Case 'Reset vor Firmenabnahme'
    $snapshot = Get-JobAgentCliSnapshot -WorkingDirectory $artifactRoot -SessionName $sessionName
    ...
    Invoke-JobAgentFilterReset -WorkingDirectory $artifactRoot -SessionName $sessionName -Case 'Reset vor Stellenpagination'
    $snapshot = Get-JobAgentCliSnapshot -WorkingDirectory $artifactRoot -SessionName $sessionName
    ...
    $searchHeading = Get-JobAgentSessionValue -WorkingDirectory $artifactRoot -SessionName $sessionName -Case 'Stellenboersenueberschrift' -Script '() => JSON.stringify({exists:!!document.getElementById("jobagent-search-heading")})'
    Assert-True -Condition ([bool]$searchHeading.exists) -Message 'Die Stellenboersenueberschrift fehlt.'
