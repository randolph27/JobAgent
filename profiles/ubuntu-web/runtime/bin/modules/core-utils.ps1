...
function Quote-PowerShellLiteral([string]$s) {
  if ($null -eq $s) { return "''" }
  return "'" + ($s -replace "'", "''") + "'"
}
...
function Run-Cmd(
  ...
  [string]$cwd=$RepoRoot,
  [hashtable]$terminalOptions=$null
) {
  ...
  try {
    if ($terminalOptions) {
      $timeoutOverride = Get-Prop $terminalOptions "default_timeout_seconds" $null
      $promptKillOverride = Get-Prop $terminalOptions "prompt_kill_timeout_seconds" $null
      $noOutputOverride = Get-Prop $terminalOptions "no_output_timeout_seconds" $null
      $heartbeatOverride = Get-Prop $terminalOptions "heartbeat_seconds" $null
      if ($null -ne $timeoutOverride -and "$timeoutOverride" -ne "") {
        $timeoutSec = [int]$timeoutOverride
      }
      if ($null -ne $promptKillOverride -and "$promptKillOverride" -ne "") {
        $promptKillSec = [int]$promptKillOverride
      }
      if ($null -ne $noOutputOverride -and "$noOutputOverride" -ne "") {
        $noOutSec = [int]$noOutputOverride
      }
      if ($null -ne $heartbeatOverride -and "$heartbeatOverride" -ne "") {
        $hbSec = [int]$heartbeatOverride
      }
    }
  } catch { $null = $_ }
  ...
  if ($psLike) {
    ...
    $psi.FileName = $psExe
    # Pass the command as plain UTF-8 input. Base64 command wrapping and policy
    # overrides are unnecessary for local CI and resemble loader behaviour.
  ...
  $null = $p.Start()
  try {
    if ($psLike) { $p.StandardInput.WriteLine([string]$cmd) }
