...
function Ensure-BootstrapFiles() {
  ...
  if (-not (Test-Path -LiteralPath $cfgPath)) {
    Write-Json $cfgPath @{
      ts=$now; features=@{ browser_tests="staged"; observer="staged" }; verify=@{ cmd=$null }; route_check="standard"; browser_smoke=@{ cmd=$null; interval_minutes=15 }; devserver=@{ cmd=$null }; pyserver=@{ cmd=$null }; todo=@{ autoseed_from_roadmap=$true; seed_max_items=8 }; deps_bootstrap=@{ auto_install=$true; allow_choco=$true }; toolchain=@{ gradle_cmd=$null; gradle_default_version="8.1.1"; java_min_major=17 }; workspace_lock=@{ on=$true; stale_minutes=30 }; gradle=@{ wrapper_autorepair=$true; wrapper_default_version="8.1.1" }
    }
  }
...
function Pin-ImmutableFiles() {
  $lock = Enter-ImmutableRuntimeLock
  try {
    Assert-ImmutableRuntimeBeforeLoad $true
    Write-ImmutablePinsBeforeLoad
    return (Read-ImmutablePinsBeforeLoad)
  } finally {
    Exit-ImmutableRuntimeLock $lock
  }
}

function Read-ImmutablePins() {
  if ([string]$env:CI_BOOTSTRAP_PREVERIFIED -eq "1") { return $null }
...
function Assert-ImmutableClean([string]$context="") {
  ...
  foreach ($f in $files) {
    ...
    $key = $rel.Replace('/','\').TrimStart('\').ToLowerInvariant()
    if (-not $key -or $pinByPath.ContainsKey($key)) {
      $issues.Add("invalid_or_duplicate_pin:" + $rel.Replace('\','/'))
      continue
    }
    $pinByPath[$key] = $f
  }
  foreach ($rel in @($ImmutableFiles | Sort-Object -Unique)) {
    $p = Join-Path $RepoRoot $rel
    if (-not (Test-Path -LiteralPath $p -PathType Leaf)) { continue }
    $key = ([string]$rel).Replace('/','\').TrimStart('\').ToLowerInvariant()
    if (-not $pinByPath.ContainsKey($key)) {
      $issues.Add("unpinned:" + ([string]$rel).Replace('\','/'))
    }
  }
  foreach ($f in $files) {
    $rel = [string](Get-Prop $f "path" "")
    ...
    if (-not (Test-Path -LiteralPath $p -PathType Leaf)) {
      $issues.Add("missing:" + $rel.Replace('\','/'))
      continue
    }
    $expectedHash = ([string](Get-Prop $f "sha256" "")).ToLowerInvariant()
    $actualHash = Sha256Path $p
    if (-not $expectedHash -or $actualHash -ne $expectedHash) {
      $issues.Add("hash_mismatch:" + $rel.Replace('\','/'))
    }
  }
  if ($issues.Count -gt 0) {
    $contextSuffix = ""
    if ($context) { $contextSuffix = " (" + $context + ")" }
    throw ("immutable verification failed" + $contextSuffix + ": " + (($issues | Sort-Object -Unique) -join ", "))
  }
  foreach ($f in $files) {
    $rel = [string](Get-Prop $f "path" "")
    if ($rel) { Set-ReadOnlyFlag (Join-Path $RepoRoot $rel) $true }
  }
}
...
function Cmd-PatchApply() {
  ...
  With-ImmutableWrite {
    ...
    $check = Run-Cmd ("git -c core.pager=cat -c color.ui=false --no-pager apply --check " + (Quote-IfNeeded $patch)) $log
    if ($check.exit -ne 0) { throw "git apply --check failed. See $log" }

    $apply = Run-Cmd ("git -c core.pager=cat -c color.ui=false --no-pager apply " + (Quote-IfNeeded $patch)) $log
    ...
    # Standalone behält Legacy-Pins; zentral verifiziert der Bootstrap nach dem Child-Lauf erneut.
    if ([string]$env:CI_BOOTSTRAP_PREVERIFIED -ne "1") {
