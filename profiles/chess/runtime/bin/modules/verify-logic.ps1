...
function Get-VerifyFailedTasks([string]$logPath) {
  if (-not (Test-Path -LiteralPath $logPath)) { return @() }
  $taskMap = @{}
  try {
    foreach ($line in (Get-Content -LiteralPath $logPath -Tail 220 -ErrorAction SilentlyContinue)) {
      $text = [string]$line
      $m = [regex]::Match($text, "Execution failed for task '([^']+)'\.")
      if ($m.Success) { $taskMap[[string]$m.Groups[1].Value] = $true }
      $m = [regex]::Match($text, '>\s*Task\s+(:[A-Za-z0-9:_-]+)\s+FAILED')
      if ($m.Success) { $taskMap[[string]$m.Groups[1].Value] = $true }
    }
  } catch {
    return @()
  }
  return @($taskMap.Keys | Sort-Object | Select-Object -First 5)
}

function Get-VerifyTestsFailedCount([string]$logPath) {
  if (-not (Test-Path -LiteralPath $logPath)) { return 0 }
  $maxFailed = 0
  try {
    foreach ($line in (Get-Content -LiteralPath $logPath -Tail 220 -ErrorAction SilentlyContinue)) {
      $text = [string]$line
      $m = [regex]::Match($text, '(\d+)\s+tests?\s+completed,\s+(\d+)\s+failed')
      if ($m.Success) {
        $failed = [int]$m.Groups[2].Value
        if ($failed -gt $maxFailed) { $maxFailed = $failed }
      }
      $m = [regex]::Match($text, 'Tests failed:\s+(\d+)')
      if ($m.Success) {
...
function Invoke-SelfCheck([string]$logPath=$null) {
  ...
  if ($null -ne $st) {
    ...
    if ($active) { $found = $false; foreach ($it in $items) { if ([string](Get-Prop $it "id" "") -eq [string]$active) { $found = $true; break } }; if (-not $found) { $issues.Add("todo_invariant: active_id not found in items (" + $active + ")") } }
  }
...
function Cmd-Verify([int]$level) {
  ...
  if ($override) {
    ...
    $g = Get-GradleCmdRaw; if (-not $g.cmd) { throw "Gradle project detected, but no Gradle command found." }; Persist-GradleInfo $g.mode $g.cmd; $cmd = ((Quote-IfNeeded $g.cmd) + " check --console=plain " + $extra).Trim(); $envMap = Get-GradleEnv; CI-Info ("verify: cmd=" + $cmd + " | log=" + $logPath); Write-VerifyDigest @{ ts=NowIso; cmd=$cmd; exit=$null; status="running"; log_path=$logPath }; $res = Run-Cmd $cmd $logPath $envMap
  } else { $cmd = "self-check"; $sc = Invoke-SelfCheck $logPath; $res = @{ exit=$sc.exit; out=""; err="" } }
  $sig = $null
  $gitLfsSignalPipe = $false
  if ($res.exit -ne 0) {
    $sig = Compute-FailSignature $logPath
    try { $gitLfsSignalPipe = Test-GitLfsSignalPipeFailure $res $logPath } catch { $gitLfsSignalPipe = $false }
  }
  $st = Load-VerifyState; $prevSig = [string](Get-Prop $st "fail_signature" $null); $prevStreak = [int](Get-Prop $st "fail_streak" 0); $streak = 0
  ...
  $failedTasks = @(Get-VerifyFailedTasks $logPath)
  $testsFailedCount = Get-VerifyTestsFailedCount $logPath
  Write-VerifyDigest @{ ts=NowIso; cmd=$cmd; exit=$res.exit; fail_signature=$sig; fail_streak=$streak; escalation_level=$level; log_path=$logPath; failed_tasks=$failedTasks; tests_failed_count=$testsFailedCount; git_lfs_signal_pipe=$gitLfsSignalPipe }
  Write-Failbundle @{ ts=NowIso; cmd=$cmd; exit=$res.exit; fail_signature=$sig; fail_streak=$streak; escalation_level=$level; reason=$(if ($gitLfsSignalPipe) { "git-lfs-signal-pipe" } else { $null }); failed_tasks=$failedTasks; tests_failed_count=$testsFailedCount; tail=(Tail-File $logPath 120); log_path=$logPath; git_lfs_signal_pipe=$gitLfsSignalPipe }
  $rep = Update-LoopGuard $cmd $sig; CI-Info ("verify: exit=" + $res.exit + " fail_signature=" + $(if ($sig) { $sig } else { "null" }) + " fail_streak=" + $streak + " log=" + $logPath)
  if ($res.exit -ne 0) {
    CI-Info "verify tail (last 30):"; (Tail-File $logPath 30) | ForEach-Object { Write-Host $_ }
    if ($rep -ge 2) { throw ("Loop-Guard: repeated failure signature for the same command (" + $sig + "). Stop retrying. Next: run #ci lock-triage if lock evidence, otherwise inspect logs/verify/failbundle-latest.json and then #ci stp.") }
    if ($gitLfsSignalPipe) { throw ("Verify failed: git-lfs signal-pipe blocker detected (Win32 error 5). Use .ci\\bin\\ci.cmd git <args> / Run-GitCmd path. log=" + $logPath) }
    throw ("Verify failed: exit=" + $res.exit + " log=" + $logPath + $(if ($sig) { " fail_signature=" + $sig } else { "" }))
  }
}
