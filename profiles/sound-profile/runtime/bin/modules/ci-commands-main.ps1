...
function New-Tst450HumanVisualReportLines([object]$summary) {
  ...
  if ([string](Get-Prop $summary "status" "") -eq "blocked") {
    ...
    $lines.Add("1. `$env:ANDROID_HOME=[IO.Path]::Combine([Environment]::GetFolderPath('LocalApplicationData'),'Android','Sdk'); `$env:ANDROID_SDK_ROOT=`$env:ANDROID_HOME")
...
function Invoke-Tst450HumanVisualSupertest([object]$gradleInfo) {
  ...
  $humanCmd = 'pwsh -NoProfile -ExecutionPolicy Bypass -File tests\emulator\human-journey\run-human-journey.ps1 -Serial "' + $serial + '" -OutputDir "' + $evidenceDir + '" -FailOnMissingRoute -FailOnPermissionDialog'
  $humanStep = Invoke-Tst450RunStep "human-journey" $humanCmd $humanLog $humanEnvMap
  $steps.Add($humanStep)
...
Register-CiCommand "event" { Cmd-Event $script:CiCommandArgs }
Register-CiCommand "tick" { Cmd-Tick $script:CiCommandArgs }
