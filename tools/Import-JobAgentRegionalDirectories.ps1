...
param(
    ...
    [Parameter()][string]$OutputPath = 'data/jobagent/company-discovery.regional-hints.json',
...
function Write-JobAgentRegionalAtomicFile {
    param(
...
function Get-JobAgentRegionalHintProperty {
    param(
...
$result = Import-JobAgentRegionalDirectories -SnapshotPath $resolvedSnapshot -SourceRegistry $registry
Write-JobAgentRegionalAtomicFile -Path $resolvedOutput -Content ($result | ConvertTo-Json -Depth 100)
...
foreach ($hint in @($existingHintStore.hints)) {
    $hintId = [string](Get-JobAgentRegionalHintProperty -Object $hint -Name 'hint_id' -Default '')
...
foreach ($hint in $mergedHints) {
    $sourceIdValue = [string](Get-JobAgentRegionalHintProperty -Object $hint -Name 'source_id' -Default 'UNKNOWN')
...
$mergedStore = [pscustomobject]@{
    ...
    generated_at = ([datetime]::UtcNow).ToString('yyyy-MM-ddTHH:mm:ss.fffZ', [Globalization.CultureInfo]::InvariantCulture)
    contract = 'Sekundaerquellen erzeugen ausschliesslich unverifizierte Discovery-Hints; sie duerfen keine JobSource und keine offizielle Karriere-URL erzeugen.'
    search_matrix_count = [int](Get-JobAgentRegionalHintProperty -Object $existingHintStore -Name 'search_matrix_count' -Default 0)
    hints_total = $mergedHints.Count
    known_company_hints = @($mergedHints | Where-Object { -not [string]::IsNullOrWhiteSpace([string](Get-JobAgentRegionalHintProperty -Object $_ -Name 'known_company_id')) }).Count
    unverified_hints = @($mergedHints | Where-Object { [string](Get-JobAgentRegionalHintProperty -Object $_ -Name 'verification_status' -Default 'UNVERIFIED') -eq 'UNVERIFIED' -or [bool](Get-JobAgentRegionalHintProperty -Object $_ -Name 'official_verification_required' -Default $false) }).Count
    source_counts = [pscustomobject]$sourceCounts
    hints = $mergedHints
}
Write-JobAgentRegionalAtomicFile -Path $resolvedMergedHints -Content ($mergedStore | ConvertTo-Json -Depth 100)

New-Item -ItemType Directory -Path $resolvedLogRoot -Force | Out-Null
$logPath = Join-Path $resolvedLogRoot ('company-discovery-regional-import-' + [datetime]::UtcNow.ToString('yyyyMMdd-HHmmss', [Globalization.CultureInfo]::InvariantCulture) + '.json')
$summary = [pscustomobject]@{
    ...
    log_path = $logPath
    sources_read = [int]$result.sources_read
