...
`tools/Verify-JobAgentCompanyCandidates.ps1` verarbeitet nur faellige `VERIFY_OFFICIAL_SITE`-Queueeintraege, schreibt produktive Firmen-/JobSource-Aenderungen ueber einen seriellen Store-Writer und legt pro Lauf ein Batchmanifest `logs/jobagent/JA-027-batch-<run-id>.json` an. Das Manifest enthaelt `run_id`, Policy, Queue-Mengen, Decision-Report, je Kandidat `batch_telemetry`, `batch_metrics`, `fetch_error_summary` und einen `resume_report`. `fetch_error_summary` aggregiert fehlgeschlagene Abrufe nach `error_class`, Kandidatenzahl, Beispiel-URLs, Beispiel-Details und `exception_types`, damit TLS-/DNS-/HTTP-Ursachen ohne erneutes Parsen aller Kandidatenlogs auswertbar sind.

`batch_metrics` nutzt ausschliesslich tatsaechlich verarbeitete eindeutige Kandidaten als Nenner fuer Dauer- und Requestwerte. P50/P95 werden per Nearest-Rank ueber sortierte Kandidatendauern berechnet. `net_official_career_growth` zaehlt nur Arbeitgeber, die nach dem seriellen Store-Commit neu als offizielle Karriere-/ATS-Quelle belegt sind; Domain-only, Fixture, Alias-Duplikate und Revalidierungen erhoehen diesen Wert nicht. Der Checkpoint speichert im `running`-Zustand geplante, erledigte und offene Kandidaten sowie ein Resultatverzeichnis. Jedes Kandidatenresultat wird vor dem Store-Commit atomar unter `data/jobagent/company-candidate-verification.checkpoint.json.results/` gesichert. Ein Folgelauf mit offenem `running`-Checkpoint laedt diese Resultate, verarbeitet nur fehlende Kandidaten nach und fuehrt anschliessend den seriellen Commit ohne Doppelimport aus. Der `completed`-Checkpoint speichert Commitzeitpunkt und dieselben Metriken wie das Batchmanifest; `logs/jobagent/JA-027-resume-<run-id>.json` dokumentiert Resume-Quelle, geladene Kandidaten und Commitstatus.
...
`tools/Test-JobAgentFetchEnvironment.ps1` prueft die TLS-Beispiel-URLs aus einem Inspection-Log mit dem produktionsnahen PowerShell/.NET-Fetchpfad, `curl.exe` und optional `wsl-curl` getrennt. Das Evidence-JSON `jobagent/fetch-environment-probe/v1` schreibt je URL die Client-Ergebnisse, Exception-Ketten und einen Status: `dotnet_fetch_available`, `dotnet_fetch_fails_but_curl_succeeds`, `schannel_fetch_fails_but_wsl_curl_succeeds` oder `all_probe_clients_failed`. Der aktuelle erfolgreiche Probe-Lauf `logs/jobagent/JA-027-fetch-environment-20260908-183047.json` meldet `dotnet_fetch_fails_but_curl_succeeds`; die produktive Auto-Policy kann daher `curl.exe` als kontrollierten Fallback nutzen. `tools/Discover-JobAgentCompanyCandidateWebsites.ps1` und `tools/Verify-JobAgentCompanyCandidates.ps1` nehmen fuer reproduzierbare Retry-Laeufe `-FetchClient auto|dotnet|curl|wsl-curl` und `-WslDistribution` entgegen und schreiben die verwendete Policy in ihre JSON-Manifeste.
...
Die produktive Manifest-Lane fuehrt derzeit auch `source-registry:stadt_muenchen_boersennotierte_unternehmen`, `source-registry:landkreis_freising_wirtschaft`, `source-registry:munich_business_international_companies`, `source-registry:munich_business_ikt_companies`, `source-registry:munich_business_life_sciences_companies`, `source-registry:munich_business_automotive_mobility_companies`, `source-registry:munich_business_finance_companies`, `source-registry:munich_business_us_companies`, `source-registry:munich_business_japanese_companies`, `source-registry:munich_business_indian_companies`, `source-registry:munich_business_creative_companies`, `source-registry:munich_business_commercial_locations`, `source-registry:munich_business_klimapakt3_companies`, `source-registry:munich_startup_platform_ecosystem`, `source-registry:awesome_ml_startups_munich_github`, `source-registry:remote_jobs_germany_github`, `source-registry:awesome_geospatial_companies_github`, `source-registry:openstreetmap_overpass_business_names`, `source-registry:tech_companies_munich_github`, `source-registry:stadt_freising_weihenstephan` und `source-registry:stadt_muenchen_unternehmensbeteiligungen` als regionale Snapshot-Quellen. Die munich-business-Quellen basieren auf oeffentlichen Standort-, Gewerbeflaechen-, Branchenfokus-, Nachhaltigkeits- und internationalen Community-Seiten. Die Boersennotierte-Muenchen-Quelle basiert auf der oeffentlichen Liste der Wirtschaftsförderung mit DAX/MDAX/SDAX/TecDAX-Unternehmen am Standort Muenchen und Region. Die Klimapakt-3-Quelle basiert auf der oeffentlichen Liste teilnehmender Muenchner Grossunternehmen und Kooperationspartner der Landeshauptstadt Muenchen. Die Munich-Startup-Quelle basiert auf dem offiziellen Startup-Portal fuer Muenchen und Region; Profilseiten werden nur als regionale Arbeitgeberhinweise genutzt, nicht als Karriere- oder Firmenwebsite-Beleg. Die Landkreis-Freising-Quelle basiert auf der oeffentlichen Wirtschaftsseite des Landkreises und fuehrt nur die dort namentlich genannten grossen Arbeitgeber. Die Tech-Companies-Munich-Quelle basiert auf einer oeffentlichen, MIT-lizenzierten GitHub-Liste mit Muenchner Tech-Unternehmen; sie wird nur als Community-Discovery-Hinweis genutzt und ihre Karriere-Links werden nicht als offizielle Anbieterlinks persistiert. Die Awesome-ML-Startups-Munich-Quelle basiert auf einer oeffentlichen GitHub-Liste mit Muenchner Machine-Learning- und AI-Startups; sie wird nur als Community-Discovery-Hinweis genutzt und externe Firmenlinks werden nicht als offizielle Anbieterlinks persistiert. Die Remote-Jobs-Germany-Quelle basiert auf einer oeffentlichen, MIT-lizenzierten GitHub-Liste mit remote-freundlichen deutschen Tech-Arbeitgebern und Muenchen-/Unterfoehring-/Ismaning-Bezug; sie wird nur als Community-Discovery-Hinweis genutzt und verlinkte Websites werden nicht als offizielle Anbieterlinks persistiert. Die Awesome-Geospatial-Companies-Quelle basiert auf einer oeffentlichen, MIT-lizenzierten GitHub-Liste mit Geo-, GIS-, Earth-Observation- und Climate-Tech-Arbeitgebern mit Muenchen- oder 20-km-Bezug; sie wird nur als Community-Discovery-Hinweis genutzt und verlinkte Firmen-, Karten- oder Sheet-Ziele werden nicht als offizielle Anbieterlinks persistiert. Die OpenStreetMap-Overpass-Quelle basiert auf einem lokalen, attributierten ODbL-Snapshot namentlich erfasster `office=company`-Elemente in Muenchen-/Freising-Bounding-Boxes; sie wird nur als koordinatenbasierter Arbeitgeberhinweis genutzt und Telefonnummern, E-Mails, Websites sowie OSM-Rohobjekte werden nicht persistiert. Die Freising-Weihenstephan-Quelle basiert auf der oeffentlichen Wirtschaftsseite der Stadt Freising; die staedtische Beteiligungsquelle basiert auf der oeffentlichen Unternehmensbeteiligungsliste der Landeshauptstadt Muenchen. Alle diese Quellen duerfen nur unverifizierte Arbeitgeberkandidaten liefern; offizielle Firmen- und Karriereverifikation bleibt Pflicht.

Beim Snapshot-Refresh bleiben bestehende Hints der erneut verarbeiteten Quellen erhalten, auch wenn sie im neuen Snapshot nicht mehr vorkommen. Solche Eintraege erhalten `retained_after_missing_source_refresh=true`, `retention_status=NOT_OBSERVED_IN_SOURCE`, `missing_since` und behalten ihre urspruengliche `observed_at`-Information. Neue oder erneut beobachtete Hints werden als `ACTIVE` markiert. Damit ist ein Quellenrefresh eine neue Beobachtung und kein Loeschgrund. Die Snapshot-Lane schreibt zusaetzlich einen Retention-Store-Update fuer `discovery_inventory` und `discovered_urls`, ohne unverifizierte Hints als produktive `companies` oder `job_sources` zu speichern.
...
Jede Quelle schreibt ein Log `logs/jobagent/company-discovery-snapshot-*.json` mit Abrufzeit, Input-Hash, Record-/Hint-Zaehlern, Reject-Gruenden, Nutzungsnotiz, Rate-Limit-Policy und `official_verification_required=true`. Der Digest `logs/jobagent/company-discovery-snapshot-digest-*.json` fasst Quellen, neue Hints, den gemergten Hint-Store und `source_gate` zusammen. Dieses Gate vergleicht alle importierbaren Snapshot-Quellen aus der Source Registry mit den eindeutig verarbeiteten Quellen und meldet fehlende oder unerwartete Quellen fail-closed als `failed`, ohne offizielle Firmen oder JobSources zu schreiben. Die Lane schreibt `data/jobagent/company-discovery.hints.json`, Snapshot-Logs und die dauerhaften Retention-Sammlungen in `data/jobagent/store.json`; `companies` und `job_sources` bleiben unveraendert.
