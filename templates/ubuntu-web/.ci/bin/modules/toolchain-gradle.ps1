...
function Download-GradleZip([string]$ver) {
  ...
  try {
    ...
    if ($expected -and $expected -match '^[0-9a-fA-F]{64}$') {
      $actual = (Get-FileHash -Algorithm SHA256 -LiteralPath $tmp).Hash.ToLowerInvariant()
      if ($actual -ne $expected.ToLowerInvariant()) { throw ("Gradle checksum mismatch. expected=" + $expected + " actual=" + $actual) }
    }
  } catch { CI-Warn ("gradle: sha256 check skipped/failed: " + $_.Exception.Message) }
...
function Ensure-LocalGradleDist([string]$ver) {
  ...
  if (-not $zip) { $zip = Download-GradleZip $ver }
  if (-not $zip) { throw ("Gradle wrapper missing and no local Gradle found. Place gradle-" + $ver + "-bin.zip into .ci\inbox\ or install Gradle on PATH.") }
...
function Get-GradleCmdRaw() {
  ...
  $root = Get-GradleProjectRoot
  if (Wrapper-Ok $root) { return @{ mode="wrapper"; cmd="gradlew.bat"; root=$root } }
