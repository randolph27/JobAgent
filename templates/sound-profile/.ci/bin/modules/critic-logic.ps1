...
function Build-CriticCapsule() {
  ...
  try {
    if (Test-Path (Join-Path $RepoRoot ".git")) {
      ...
      $git.status = (Run-Cmd "git -c core.pager=cat -c color.ui=false --no-pager status --porcelain=v1" $log).out; $git.diff_stat = (Run-Cmd "git -c core.pager=cat -c color.ui=false --no-pager diff --stat" $log).out; $git.changed_paths = Try-GitDiffNames $log
      $patch = (Run-Cmd "git -c core.pager=cat -c color.ui=false --no-pager diff" $log).out; if ($patch -and $patch.Length -gt 50000) { $patch = $patch.Substring(0,50000) + "`n...[truncated]..." }; $git.diff_patch = $patch
    }
  } catch { $null = $_ }
...
function Cmd-Autopatch() {
  ...
  if ($autoCommit -and (Test-Path (Join-Path $RepoRoot ".git"))) { $log = Join-Path $LogsRoot ("terminal\autopatch-git-" + $ts + ".log"); Run-Cmd "git add -A" $log | Out-Null; $msg = "ci(autopatch): apply critic patch"; Run-Cmd ("git -c core.pager=cat -c color.ui=false --no-pager commit -m " + (Quote-IfNeeded $msg)) $log | Out-Null; if ($autoPush) { Run-Cmd "git -c core.pager=cat -c color.ui=false --no-pager push" $log | Out-Null } }
}
