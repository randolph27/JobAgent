...
<!-- BEGIN FULL PROFILE CONTRACT: ubuntu-web -->

## Vollstaendiger Profilvertrag: Ubuntu Web / Quiz

Quelle: D:\_Scripte\UbuntuVPS_Quiz\web, Git-HEAD 31e53354f0c50292c00dda94c30efeea46513512, aus Git-HEAD. Dieser Abschnitt ist vollstaendig; die einzige redaktionelle Aenderung ist gegebenenfalls ein markierter fehlender Markdown-Fence-Abschluss.
...
# 3) Cleanup
Remove-Item .ci/run/devserver.pid.json -ErrorAction SilentlyContinue
<!-- BOOTSTRAP-FENCE-REPAIR: fehlenden Abschluss vor Unix-Beispiel ergaenzt -->
```
Unix (Start, detached):
...
<!-- END FULL PROFILE CONTRACT: ubuntu-web -->

<!-- BEGIN FULL PROFILE CONTRACT: chess -->

## Vollstaendiger Profilvertrag: Chess / Kotlin-Multiplatform / Viewport

Quelle: D:\_Scripte\Chess, Git-HEAD fef812316c54be6d9170f7d63712b823671c4586, aus Git-HEAD. Dieser Abschnitt ist vollstaendig; die einzige redaktionelle Aenderung ist gegebenenfalls ein markierter fehlender Markdown-Fence-Abschluss.
...
1) Startparameter
workspace_root: <D:\_Scripte\Chess>
mode: prod
agent_id: auto
preflight: strict
manual: auto
...
Rehydrate (Start jeder Iteration):
	• Reihenfolge: README → Roadmap.md → Roadmap_archive.md → Roadmap_index.md → Contract → Manual (falls vorhanden) → env-inventory (falls vorhanden) → handoff → todo-state.
	• Gate: Wenn README/Roadmap-Dateien in dieser Session noch nicht gelesen wurden, müssen sie vor jedem anderen Read nachgeladen werden.
	• Wenn aktive Roadmap-Punkte `Screenshot-Referenzen:` enthalten, muessen die dort genannten Dateien unter `docs/roadmap_screenshots/` im selben Rehydrate-Schritt genau einmal eingelesen und gecacht werden.
	• Manual dient Anforderungen/Policies/Bedienung; env-inventory dient ausschließlich der Tool-/Command-Auswahl.
Chat-Start (zusätzlich, genau einmal):
1. Preflight (siehe 7.1.1): nur read-only Checks + ggf. gradlew --stop.
	2. README.md lesen und cachen (Pflicht).
	...
	4. Roadmap_archive.md lesen und cachen (Pflicht).
	5. Roadmap_index.md lesen und cachen (Pflicht).
	5a. Wenn `Roadmap.md` aktive Punkte mit `Screenshot-Referenzen:` enthaelt: die referenzierten Dateien unter `docs/roadmap_screenshots/` lesen und cachen (Pflicht).
	6. architecture.contract.md lesen (falls vorhanden).
	7. Manual einmal ingestieren (falls vorhanden) und manual.digest.json erzeugen/aktualisieren (Single-Line-JSON, kurz; Metadaten+Hash, siehe 1).
	8. env-inventory einmal einlesen (falls vorhanden) and env_inventory.digest.json erzeugen/aktualisieren (Single-Line-JSON, kurz).
	9. Observer-Baseline sicherstellen (siehe 9):
...
Windows PowerShell (Start, detached, guard+cleanup):
# Guard: falls PID-Datei existiert oder Port 8080 belegt ist → stop
if (Test-Path .ci/run/devserver.pid.json) { try { $j = Get-Content -Raw .ci/run/devserver.pid.json | ConvertFrom-Json; Stop-Process -Id $j.pid -Force -ErrorAction SilentlyContinue } catch {} ; Remove-Item .ci/run/devserver.pid.json -ErrorAction SilentlyContinue }
$pidExisting = (Get-NetTCPConnection -LocalPort 8080 -State Listen -ErrorAction SilentlyContinue | Select-Object -First 1 -ExpandProperty OwningProcess)
if ($pidExisting) { Stop-Process -Id $pidExisting -Force -ErrorAction SilentlyContinue }
...
if (Test-Path $log) { if ((Get-Item $log).Length -gt 200kb) { Move-Item $log ("logs/devserver/devserver-"+(Get-Date -Format "yyyyMMdd-HHmmss")+".log") } }
$p = Start-Process -FilePath "powershell" -ArgumentList "-NoProfile","-ExecutionPolicy","Bypass","-Command",$cmd -WorkingDirectory (Get-Location) -RedirectStandardOutput $log -RedirectStandardError $log -PassThru -WindowStyle Hidden
@{ ts=(Get-Date).ToString("o"); pid=$p.Id; cmd=$cmd; cwd=(Get-Location).Path; port=8080; url="http://localhost:8080/" } | ConvertTo-Json -Compress | Set-Content -NoNewline .ci/run/devserver.pid.json
Windows PowerShell (Stop, inkl. Fallback):
```powershell
# 1) Primär: PID-Datei
if (Test-Path .ci/run/devserver.pid.json) {
...
# 2) Fallback: Port 8080 (webpack-dev-server Default)
$pid = (Get-NetTCPConnection -LocalPort 8080 -State Listen -ErrorAction SilentlyContinue | Select-Object -First 1 -ExpandProperty OwningProcess)
if ($pid) { Stop-Process -Id $pid -Force -ErrorAction SilentlyContinue }
# 3) Cleanup
Remove-Item .ci/run/devserver.pid.json -ErrorAction SilentlyContinue
<!-- BOOTSTRAP-FENCE-REPAIR: fehlenden Abschluss vor Unix-Beispiel ergaenzt -->
...
nohup ./gradlew :web:jsBrowserDevelopmentRun --console=plain >>"$log" 2>&1 &
pid=$!
printf '{"ts":"%s","pid":%s,"cmd":"%s","cwd":"%s","port":8080,"url":"http://localhost:8080/"}' "$(date -Iseconds)" "$pid" "./gradlew :web:jsBrowserDevelopmentRun --console=plain" "$(pwd)" > .ci/run/devserver.pid.json
Unix (Stop):
pid=$(grep -o '"pid":[0-9]*' .ci/run/devserver.pid.json 2>/dev/null | head -n1 | cut -d: -f2)
[ -n "$pid" ] && kill -9 "$pid" 2>/dev/null || true
rm -f .ci/run/devserver.pid.json
...
Wenn eine Aenderungsanforderung bestehendes Produktverhalten anfasst, muessen im selben Arbeitsgang die betroffenen Funktionstests und, falls die Fehlerklasse regressionskritisch ist, auch die Supertest-Verankerung aktualisiert werden. Produktfix ohne Testnachzug gilt nicht als abgeschlossen.
...
Prüfe, ob Dev Server auf:8080 läuft. Wenn nicht, starte ihn und lasse ihn laufen.

## Browser-Viewport-Handshake bei wiederkehrendem UI-Drift (hart)
- Wenn ein User einen sichtbaren UI-Fehler meldet und mindestens einer der folgenden Trigger zutrifft, gilt der Fall sofort als `viewport-mismatch-risk`:
  - derselbe Fehler bleibt trotz gruener fokussierter Guards oder lokal gruener Fixes sichtbar,
  - zwei Chats behandeln denselben Fehler mit unterschiedlichen lokalen Green-Paths,
  - der vorhandene Test- oder Evidence-Bestand deckt den gemeldeten Browserzustand nicht exakt ab.
- In diesem Zustand darf kein lokaler Green-Path als fachlich ausreichend gelten. Vor Red-Gate oder Produktfix muss der reale User-Viewport verifiziert werden.
- Pflichtdaten fuer die Rueckfrage:
  - technischer Viewport des Browserinhalts (`width x height`),
  - Browser plus Rendering-Kontext (`Chrome`/`Edge`/`Firefox`, normales Browserfenster oder DevTools-Device-Mode),
  - Zoom-Prozent,
  - `window.devicePixelRatio` oder OS-Display-Skalierung,
  - optional, aber bevorzugt: Screenshot der DevTools-Viewportleiste oder des kompletten Browserfensters.
- Ein vorhandener Testviewport gilt nur dann ohne Rueckfrage als ausreichend aehnlich, wenn alle folgenden Punkte gleichzeitig stimmen:
  - gleiche Orientierung (`landscape`/`portrait`),
  - gleiche Browser-Chrome-Klasse (`echtes Browserfenster`, `Device-Mode`, `Standalone/PWA`),
  - gleiche Zoom-Klasse (`100%` vs. ungleich `100%`),
  - gleiche DPR-/Skalierungs-Klasse (`1.0`, `1.25-1.5`, `>=2.0`),
  - Breiten- und Hoehendelta jeweils hoechstens `32px`.
- Sobald der Viewport verifiziert ist, ist die Reihenfolge fest:
  1. passenden fokussierten Red-Gate fuer genau diesen Viewport bauen oder erweitern,
  2. Produktfix nur gegen diesen Guard umsetzen,
  3. denselben Viewport dauerhaft in der passenden Funktionstest-Matrix behalten,
  4. Handoff und Roadmap mit dem verifizierten Viewport als Referenzzustand aktualisieren.
- Ausloeserfall fuer diesen Mechanismus: `1440x900 @ 100%` blieb sichtbar rot, obwohl lokale Guards zuvor gruen waren.
...
### Screenshot-Prozedur (allgemein, hart)
- Wenn Roadmap-Punkte aus User-Screenshots, Browser-Screenshots oder Bildvergleichen entstehen, werden die Referenzbilder unter `docs/roadmap_screenshots/<roadmap-id>/` abgelegt.
- Pro Bild ist ein stabiler Dateiname zu verwenden, z.B. `01-right-rail-analyze-1920x1080.png`, `02-left-glicko-crop-1366x900.png`.
- Wenn ein Bild nicht direkt als Binary ins Repo geschrieben werden kann, muss im selben Ordner mindestens eine `manifest.md` liegen, die jede Referenz mit Dateinamen, Quelle, Zweck und TODO zum Nachpflegen des Binarys festhaelt; ein neuer Chat muss diese Datei trotzdem mit einlesen.
- Jeder aktive Roadmap-Punkt mit visuellem Bezug muss eine eigene Sektion `Screenshot-Referenzen:` enthalten, die auf die Dateien oder die `manifest.md` unter `docs/roadmap_screenshots/<roadmap-id>/` verweist.
- Bei Chat-Start und Rehydrate muessen diese `Screenshot-Referenzen:` zusammen mit `Roadmap.md` eingelesen werden; ohne dieses Einlesen darf keine visuelle Umsetzung oder Test-Haertung beginnen.

### Antwortregel fuer Beweisdateien (hart)
- Wenn der User einen Dateipfad zu einer Beweisdatei, Screenshot-Datei oder Evidence-Datei anfordert, muss die Antwort einen echten lokalen Dateipfad aus dem Workspace oder dem explizit genannten Speicherort liefern.
- Verboten sind Webview-/Browser-/VSCode-Ressourcen wie `https://...`, `file+...`, `vscode-resource:...`, `vscode-cdn.net/...` oder sonstige URL-Formate, auch wenn sie im UI anklickbar erscheinen.
- Erwartet ist ein konkreter Dateipfad, der typischerweise mit einer echten Dateiendung endet, z. B. `.png`, `.jpg`, `.json`, `.md`, `.txt`.
- Wenn klickbare Links ausgegeben werden, muessen sie direkt auf die echte Datei zeigen und im Linkziel selbst mit der echten Dateiendung enden; ein Verzeichnis, ein `/webview/`-Pfad, ein CDN-Wrapper oder ein umgeleiteter Containerpfad ist unzulaessig.
- Fuer lokale Dateien ist das Markdown-Zielformat strikt: `[name.ext](</absoluter/pfad/name.ext>)` beziehungsweise mit Zeilennummer `[name.ext](</absoluter/pfad/name.ext:12>)`. Labels ohne Dateiendung oder Ziele, die nicht auf die echte Datei aufloesen, sind unzulaessig.
- Wenn ein Client oder Renderer einen Link auf eine Webview-/CDN-URL umbiegt, gilt das Ergebnis als kaputt; der Agent muss dann stattdessen den nackten absoluten Dateipfad oder einen korrekt gerenderten lokalen Markdown-Dateilink liefern, aber niemals den kaputten Webview-Link wiederholen.
- Wenn mehrere Artefakte existieren, sind alle relevanten lokalen Dateipfade einzeln auszugeben; kein Ersetzen durch Sammel-Links, Webview-Container oder generische Ordnerhinweise.
- Wenn noch keine Datei existiert, darf kein Scheinpfad ausgegeben werden; stattdessen erst die Datei erzeugen und danach den echten Dateipfad nennen.
...
- Unterpunkte (alle checkboxbar, eingerueckt):
  1. `Beschreibung:` Was genau soll am Ende anders sein (messbar, keine Adjektive ohne Kriterium).
     - Darunter **genau 3** detaillierte, checkboxbare Beschreibungs-Unterpunkte (`Teilbild 1`, `Teilbild 2`, `Teilbild 3`) mit klarer visueller/funktionaler Aussage.
  2. `Screenshot-Referenzen:` Konkrete Pfade unter `docs/roadmap_screenshots/<roadmap-id>/`, die fuer den Punkt eingelesen werden muessen.
  3. `Scope:` Betroffene Dateien/Module/Tests (Pfad/Dateinamen), explizite No-Gos (was NICHT aendern).
  4. `Ist-Stand (YYYY-MM-DD HH:MM):` Beobachtung + betroffene Tests/Fehler (wenn unbekannt: als TODO markieren, nicht erfinden).
  5. `Schritte:` Nummerierte Liste mit **mindestens 3** konkreten Umsetzungsschritten (je Schritt: Aktion + betroffene Datei/Komponente + erwartetes Ergebnis).
  6. `Evidence:` Welche Artefakte am Ende existieren muessen (Screens/Logs/Pfade, Dateiname mit ID).
  7. `Funktionstest:` Exakte Commands/Tests (z.B. `node tests/ui/<...>.js`).
  8. `Audit:` Manuell-visueller Check (mind. 1920/1366/800), klare Akzeptanzkriterien (kein Overlap/Clipping, Touch-Targets, Typo).
  9. `Supertest:` Feld immer ausfuellen; Lauf nur auf expliziten User-Befehl (kein Automatismus nach jedem Roadmap-Punkt).
...
- Ausgabe NUR als Markdown-Roadmap-Items (Checkboxen), keine Erklaertexte.
- Pro Hauptpunkt: Hauptzeile mit `#comment:` + Unterpunkte in dieser Reihenfolge:
  Beschreibung (mit genau 3 detaillierten Beschreibungs-Unterpunkten), Screenshot-Referenzen, Scope, Ist-Stand (YYYY-MM-DD HH:MM), Schritte (min. 3, nummeriert), Evidence, Funktionstest, Audit, Supertest.
- Jeder Unterpunkt ist checkboxbar und enthaltene Anforderungen sind messbar.
- Wenn visuelle Referenzen existieren: lege sie unter `docs/roadmap_screenshots/<roadmap-id>/` ab und referenziere sie explizit.
- Keine erfundenen Facts: Wenn ein Detail unbekannt ist, schreibe `TODO: <was zu ermitteln ist>`.
- Platform/CI-Guardrails einhalten (Web/PWA only, keine android.* / androidx.* Imports).
...
### Output-Selbstcheck (Agent muss vor Abgabe erfuellen)
- [ ] Jeder Punkt hat `#comment:` (1 Satz, laesst keinen Interpretationsspielraum).
- [ ] `Beschreibung` hat genau 3 detaillierte checkboxbare Unterpunkte.
- [ ] `Screenshot-Referenzen` ist vorhanden und zeigt auf konkrete Pfade unter `docs/roadmap_screenshots/<roadmap-id>/` oder auf eine `manifest.md`.
- [ ] `Schritte` hat >= 3 Schritte; jeder Schritt nennt Aktion + Datei/Komponente + erwartetes Resultat.
- [ ] `Evidence` nennt konkrete Pfade/Dateinamen mit ID.
- [ ] `Funktionstest` listet konkrete Commands (keine "Tests laufen lassen").
- [ ] `Audit` und `Supertest` sind IMMER vorhanden und am Ende; der Supertest-Lauf selbst erfolgt nur auf expliziten User-Befehl.
- [ ] Keine neuen Architekturentscheidungen; kein Drift gegen README/Roadmap-Guardrails.

<!-- END FULL PROFILE CONTRACT: chess -->

<!-- BEGIN FULL PROFILE CONTRACT: sound-profile -->

## Vollstaendiger Profilvertrag: Sound Profile / Android / ADB

Quelle: D:\_Scripte\Sound Profile, Git-HEAD 7d6ad8f9eb0d6de1874554e15f4234ae0ba98cfa, aus Git-HEAD. Dieser Abschnitt ist vollstaendig; die einzige redaktionelle Aenderung ist gegebenenfalls ein markierter fehlender Markdown-Fence-Abschluss.

## Rollenhinweis

Diese Datei ist der CI-/Agent-Entry fuer Arbeitsfluss, Todo-System, Route-Check, Git und Hintergrundprozesse; sie ist kein Produkt-README. Produktueberblick und aktueller Status stehen in `manual/index.md` und `manual/summary.md`.
...
1) Startparameter
workspace_root: <D:\_Scripte\Sound Profile>
mode: prod
agent_id: auto
preflight: strict
manual: auto
...
2) Quellenhierarchie
	...
	6. architecture.contract.md (falls vorhanden; bindend)
3.1 browser-tests.contract.md (falls vorhanden; Browser-Test-Definition, bindend)
	8. manual/ oder manual.md (falls vorhanden; Vorgaben/Bedienung/Policies; immutable außer expliziter Änderung)
	9. env-inventory.snapshot.md (falls vorhanden; Umgebungsfakten: Tooling/Paths)
	10. handoff.latest.json (Resume-Anker)
	11. todo.events.jsonl (History, Truth)
	12. todo.state.json (aktueller Zustand, Truth)
	13. todo.current.md (View, kurz)
	14. Debug-/Sonst-Logs (Belege; keine Entscheidungsgrundlage)
Konfliktregel: höhere Priorität gewinnt.
...
1. Preflight (siehe 7.1.1): nur read-only Checks + ggf. gradlew --stop.
	...
	5. Roadmap_index.md lesen und cachen (Pflicht).
	6. architecture.contract.md lesen (falls vorhanden).
	8. Manual einmal ingestieren (falls vorhanden) und manual.digest.json erzeugen/aktualisieren (Single-Line-JSON, kurz; Metadaten+Hash, siehe 1).
	9. env-inventory einmal einlesen (falls vorhanden) and env_inventory.digest.json erzeugen/aktualisieren (Single-Line-JSON, kurz).
	10. Observer-Baseline sicherstellen (siehe 9):
		○ wenn observer: staged und observer.baseline.json fehlt: Baseline erzeugen.
		○ einmaliger observer-check ausführen.
	11. Aktion todo-compact durchführen, wenn Trigger erfüllt (siehe 5.5).
	12. Aktion todo-prune durchführen, wenn Pruning-Regel erfüllt ist (siehe 5.6).
	13. Aktion todo-rotate durchführen, wenn Rotation-Trigger erfüllt ist (siehe 5.6).
	14. todo.master.index.json einmal lesen (Überblick), danach nur bei Dateiänderung.
	15. todo.history.digest.json einmal lesen (Überblick über erledigte Tasks), danach nur bei Dateiänderung.
	16. Wenn browser_tests: staged: #ci browser-smoke prüfen/ausführen (siehe 7.1.3).
Gate:
	• Vor Schritt 1–5 dürfen keine anderen Dateien gelesen/geschrieben und keine Aktionen ausgeführt werden.
	• Ausnahme: Schritt 1 (Preflight) ist erlaubt und zählt nicht als Drift.
Regeln:
...
## ADB-Geraeteverbindung (hart)
- Ein ADB-Connect/Geraeteauswahl pro Testlauf-Session reicht aus; nicht vor jeder Aktion neu verbinden.
- Nachfolgende `adb`-Befehle (install, shell, dumpsys, settings get) in derselben Session koennen dieselbe Serial wiederverwenden.
- Kein `adb kill-server`/`adb start-server`-Zyklus pro Testschritt, ausser zur expliziten Fehlerbehebung (z.B. `offline`/`unauthorized`).
...
<!-- END FULL PROFILE CONTRACT: sound-profile -->
