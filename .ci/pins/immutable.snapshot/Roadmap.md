...
- Review-Basis: Implementierung bis JA-015 ist weitgehend fachlich angelegt, aber die angehängte Programmanweisung ist noch nicht vollständig umgesetzt. Gesichert vorhanden sind Persistenz, Firmen-Seed, offizielle Quellenverifikation, Fixture-/generischer HTML-/Live-Adapter, Klassifikation, Deduplikation, Statusmaschine, Daily-Run, Markdown-/JSON-Report, Betriebsstatus, Coverage-Backlog und Supertest.
- Kritische Lücken: HTML-Output fehlt; Reportfelder aus der Programmanweisung sind unvollständig; Live-Recherche ist noch Pilotqualität; Firmeninventar wird nicht systematisch autonom erweitert; Status `CLOSED` und quellenbezogene Entfernungssicherheit sind noch nicht ausreichend; lokale App-/Artefaktablage und Audit-Nachweise sind noch nicht vollständig vertraglich abgesichert.
- Priorisierung: zuerst HTML-Output und Report-Vertragskonformität, weil sie direkt vom Nutzer gefordert und für Ergebnisnutzung sichtbar sind; danach Status-/Quellensicherheit, weil falsche NEW/REMOVED/CLOSED-Ergebnisse fachlich schwerer wiegen; danach Live-Adapter und Firmenabdeckung; zuletzt lokale Betriebs-/Audit-Härtung.
...
- [ ] JA-016 Lokalen HTML-Report als Daily-Run-Artefakt erzeugen und in `html/` ablegen #comment: Die Nutzeranforderung verlangt zusätzlich zum bestehenden JSON-/Markdown-Report einen lokal öffnbaren HTML-Output im Projektverzeichnis.
  - [ ] Beschreibung: Am Ende jedes `Invoke-JobAgentDailyRun` existiert neben `logs/jobagent/daily-run-<stamp>.json` und `.md` ein HTML-Bericht unter `html/jobagent/daily-run-<stamp>.html`; der Rückgabewert, Summary-JSON und `scan_run.artifact_paths` enthalten diesen relativen oder absoluten Pfad deterministisch.
  - [ ] Scope: Betroffen sind `src/JobAgent.Report.psm1`, `src/JobAgent.DailyRun.psm1`, `tests/Test-JobAgentReport.ps1`, `tests/Test-JobAgentDailyRun.ps1`, `tests/Test-JobAgentSupertest.ps1` und optional `tools/Invoke-JobAgentDailyRun.ps1`; No-Go: kein externer CDN-Link, kein Devserver-Zwang, kein JavaScript für Kerninhalt, keine unescaped HTML-Ausgabe.
  - [ ] Ist-Stand (2026-08-17 16:20): JSON- und Markdown-Reports werden erzeugt; ein HTML-Verzeichnis existiert lokal, aber der Daily-Run schreibt kein HTML-Artefakt und kein Test fordert `html/jobagent/*.html`.
  - [ ] Abhängigkeiten: JA-010 und JA-011 sind abgeschlossen; JA-016 kann ohne Live-Webrecherche umgesetzt werden.
  - [ ] Aufwand/Dauer: Aufwand M; Dauer 0,5-1 Arbeitstag bei 1 Agent; parallelisierbar mit JA-017 nur nach Festlegung des gemeinsamen Report-Viewmodels.
  - [ ] Prioritätsscore: 100/100, weil dies eine explizite aktuelle Nutzeranforderung ist und die lokale Nutzbarkeit des Ergebnisses blockiert.
  - [ ] Ordnungsbegründung: HTML-Ausgabe ist sichtbares Ergebnisartefakt und sollte vor weiteren Live-Recherche-Erweiterungen existieren, damit spätere Qualitätsverbesserungen sofort in derselben Ausgabeform geprüft werden können.
  - [ ] Risiken und Unsicherheiten: HTML-Encoding, URL-/Text-Escaping und Tabellenbreite können bei langen Titeln oder URLs zu Layoutproblemen führen; unklar ist, ob zusätzlich ein stabiler Alias wie `html/jobagent/latest.html` gewünscht ist, daher als optionale Kopie mit Test absichern oder bewusst nicht implementieren.
  - [ ] Schritte:
    1. `ConvertTo-JobAgentDailyReportHtml` in `src/JobAgent.Report.psm1` implementieren, mit HTML5-Grundgerüst, escaped Textzellen, semantischen Sektionen nach Reportvertrag, lokaler CSS-Definition und ohne externe Ressourcen.
    2. `Write-JobAgentDailyRunHtmlReport` in `src/JobAgent.DailyRun.psm1` ergänzen, relativen Pfad `html/jobagent/daily-run-<stamp>.html` erzeugen, atomar schreiben und `scan_run.artifact_paths`, Summary und CLI-Ausgabe erweitern.
    3. Tests für Existenz, Pfad unter Projektroot, Pflichtüberschriften, Escaping von `<script>`, lange URLs/Titel, leeren Zustand und Summary-Felder ergänzen.
  - [ ] Evidence: `html/jobagent/daily-run-<stamp>.html`, Testausgabe von `tests/Test-JobAgentReport.ps1`, Testausgabe von `tests/Test-JobAgentDailyRun.ps1`, optional `logs/jobagent/daily-run-<stamp>.json` mit `html_report_path`.
  - [ ] Funktionstest: `pwsh -NoProfile -File tests\Test-JobAgentReport.ps1`; `pwsh -NoProfile -File tests\Test-JobAgentDailyRun.ps1`; beide müssen exit=0 liefern und das HTML-Artefakt in einem temporären Projektroot prüfen.
  - [ ] Audit: HTML-Datei lokal im Browser oder per statischem Parser prüfen; Akzeptanz bei 1920/1366/800 px: keine abgeschnittenen Tabellenüberschriften, lange Links umbrechen, Priorität/Status visuell unterscheidbar, keine überlappenden Texte, kein externer Netzwerkabruf.
  - [ ] Supertest: Erst nach grünen Funktionstests `pwsh -NoProfile -File tests\Test-JobAgentSupertest.ps1`; anschließend `.\ci.cmd self-check`.
  - [ ] Meilenstein und Parallelisierung: M6; nach gemeinsamem Report-Viewmodel parallel mit JA-017 teilweise möglich, aber Commit-Reihenfolge zuerst JA-016, damit der HTML-Output als Zielartefakt existiert.
...
- [ ] JA-017 Reportfelder vollständig gegen Programmanweisung und Daily-Run-Ausgabeformat schließen #comment: Der bestehende Report erfüllt die Struktur grob, lässt aber mehrere Pflichtfelder aus der angehängten Anweisung nur indirekt oder gar nicht sichtbar werden.
  ...
  - [ ] Abhängigkeiten: JA-016 sollte zuerst oder im selben Schnitt das gemeinsame HTML-/Markdown-Viewmodel bereitstellen; JA-002 bis JA-011 sind abgeschlossen.
  - [ ] Aufwand/Dauer: Aufwand M-L; Dauer 1 Arbeitstag bei 1 Agent; parallelisierbar mit JA-019 bei getrennten Tests, aber nicht mit JA-016 ohne abgestimmtes Viewmodel.
...
- [ ] JA-022 Lokale App-/Artefaktablage, Devserver-Port und Visual-Audit für HTML-Berichte absichern #comment: Die App läuft nur lokal; Berichte müssen reproduzierbar abgelegt, geöffnet und visuell geprüft werden können.
  ...
  - [ ] Abhängigkeiten: JA-016 muss HTML-Artefakte erzeugen; JA-017 sollte die sichtbaren Pflichtfelder liefern.
  - [ ] Aufwand/Dauer: Aufwand M; Dauer 0,5-1 Arbeitstag bei 1 Agent; nach JA-016/017 weitgehend unabhängig von Live-Adapter-Arbeit.
