function Get-BrowserTestsMode() {
  $cfg = Try-ReadJson (Get-ConfigPath)
  $features = Get-Prop $cfg "features" $null
  return ([string](Get-Prop $features "browser_tests" "off")).ToLowerInvariant()
}

function Write-BrowserSmokeDigest([string]$status, [string]$reason, [int]$exit=0, [string]$cmd=$null, [string]$url=$null) {
  $payload = @{
    ts = NowIso
    status = $status
    reason = $reason
    ...
    cmd = $cmd
    url = $url
    contract = "browser-tests.contract.md"
  }
  Write-Json (Join-Path $LogsRoot "verify\browser.smoke.digest.json") $payload
}

function Get-BrowserSmokeContractCommand([string]$contractPath) {
  if (-not (Test-Path -LiteralPath $contractPath)) { return $null }
  $lines = @()
  try { $lines = Get-Content -LiteralPath $contractPath -ErrorAction SilentlyContinue } catch { $lines = @() }
  for ($i=0; $i -lt $lines.Count; $i++) {
    $t = ([string]$lines[$i]).Trim()
    if (-not $t) { continue }
    if ($t -match '^(?i)(cmd|command)\s*:\s*(.+)$') {
    ...
    if ($t -match '^(?i)(cmd|command)\s*:\s*$' -or $t -match '^(?i)<cmd>\s*$') {
      if (($i+1) -lt $lines.Count -and ([string]$lines[$i+1]).Trim().StartsWith('```')) {
        ...
        while ($j -lt $lines.Count) {
          $x = [string]$lines[$j]
          if ($x.Trim().StartsWith('```')) { break }
          if ($x.Trim() -and -not $x.Trim().StartsWith("#")) { $buf.Add($x) }
          $j++
        }
        $candidate = ($buf -join "`n").Trim()
...
function Test-BrowserSmokeContractInactive([string]$contractPath) {
  if (-not (Test-Path -LiteralPath $contractPath)) { return $false }
  $text = ""
  try { $text = (Get-Content -Raw -LiteralPath $contractPath).ToLowerInvariant() } catch { return $false }
  return (
    ($text -match 'inaktiv|kein browser-smoke-command|keine web-oberflaeche') -and
    (-not (Get-BrowserSmokeContractCommand $contractPath))
  )
}

function Get-BrowserSmokeInactiveReason() {
  $mode = Get-BrowserTestsMode
  if ($mode -eq "off") { return "browser_tests_off" }
  $contract = Join-Path $RepoRoot "browser-tests.contract.md"
  if (Test-BrowserSmokeContractInactive $contract) { return "browser_contract_inactive_native_android" }
  return $null
}

function Cmd-BrowserSmoke() {
  Ensure-CoreFolders; Ensure-BootstrapFiles
  $inactiveReason = Get-BrowserSmokeInactiveReason
  if ($inactiveReason) {
    Write-BrowserSmokeDigest "skipped" $inactiveReason 0 $null $null
    CI-Info ("browser-smoke: skipped (" + $inactiveReason + ")")
    return
  }
  ...
  if (-not $cmd) {
    $contract = Join-Path $RepoRoot "browser-tests.contract.md"
    $cmd = Get-BrowserSmokeContractCommand $contract
  }
...
function Cmd-DevserverStart() {
  ...
  if ($cmdOverride) {
    ...
    $task = Get-Prop $ds "task" $null
    if (-not $task) { throw "devserver-start: Gradle-Projekte brauchen devserver.cmd oder devserver.task; der native Android-Default startet keinen Web-Devserver." }
  ...
  $header = "## $(TsId)`r`nCMD: $cmd`r`nPWD: $($devCwd)`r`n"; Set-Content -LiteralPath $logPath -Value $header -Encoding UTF8
