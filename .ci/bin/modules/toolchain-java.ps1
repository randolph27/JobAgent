...
function Read-ToolchainPins() {
  ...
  $keyAliases = @{
    java = @("java", "jdk", "java_major")
    gradle = @("gradle", "gradle_version")
    node = @("node", "node_version")
  }
  foreach ($targetKey in @("java","gradle","node")) {
    foreach ($sourceKey in $keyAliases[$targetKey]) {
      $m = [regex]::Match($t, '(?im)^\s*-\s*`?' + [regex]::Escape($sourceKey) + '`?\s*:\s*`?([0-9][0-9\.]*)`?\s*$')
      if ($m.Success) {
        $pins[$targetKey] = $m.Groups[1].Value.Trim()
        break
      }
    }
  }
